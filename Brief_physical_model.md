...
## Physical model map[​](#physical-model-map "Direct link to Physical model map")

A detailed map of a single area — how data is stored in the database. The overall element map is in [Brief](/Brief/.md), and the mechanisms are described in [Tables](/Tables/.md), [Materializations](/Materializations/.md) and [Indexes](/Indexes/.md).

This article is not part of the set always passed to the assistant; it is requested when needed.

## What goes into the database[​](#what-goes-into-the-database "Direct link to What goes into the database")

* Data ([`DATA`](/Data_properties_DATA/.md)) properties — always stored.
...
* A table is declared by the [`TABLE` statement](/TABLE_statement/.md) with the list of its key classes: `TABLE skuStock (Sku, Stock);`.
...
## Names in the database[​](#names-in-the-database "Direct link to Names in the database")

Table and field names depend on the naming policy set by the [`db.namingPolicy`](/Launch_parameters/.md#namingpolicy) launch parameter.
...
* An index on one property is set by the `INDEXED` option; an index on an arbitrary list of fields of one table is created by the [`INDEX` statement](/INDEX_statement/.md).
...
## Recalculation[​](#recalculation "Direct link to Recalculation")

The stored values of a materialized property are recomputed from its definition by the [`RECALCULATE` operator](/RECALCULATE_operator/.md) — this is needed when the values may have diverged from the definition, for example after the definition changed or after a direct data fix.
