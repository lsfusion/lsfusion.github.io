...
## Brief on lsFusion elements for AI agents[​](#brief-on-lsfusion-elements-for-ai-agents "Direct link to Brief on lsFusion elements for AI agents")
...
## Domain logic — the core elements (from simpler to more complex)[​](#domain-logic--the-core-elements-from-simpler-to-more-complex "Direct link to Domain logic — the core elements (from simpler to more complex)")
...
* **Statement** ([`CLASS`](/CLASS_statement/.md)): `CLASS ABSTRACT name [caption] [imageSetting] [: parent1, ..., parentN];` and `CLASS [NATIVE] name [caption] [imageSetting] [{ objectName1 [objectCaption1] [imageSetting], ... }] [: parent1, ..., parentN];` for a class with static objects
...
* **Statement** ([`=`](/=_statement/.md)): `name [caption] [(param1, ..., paramN)] = expression [options];`; a stored one is `= DATA [LOCAL [NESTED [MANAGESESSION | NOMANAGESESSION]]] returnClass [(argumentClass1, ..., argumentClassN)]` ([`DATA`](/DATA_operator/.md)).
...
* **Statement** ([`ACTION`](/ACTION_statement/.md)): `name [caption] [(param1, ..., paramN)] { actionBody } [options]`
...
* **Statements** ([`WHEN`](/WHEN_statement/.md), [`ON`](/ON_statement/.md)): `WHEN eventClause eventExpr [ORDER [DESC] orderExpr1, ..., orderExprN] DO eventAction;` and `ON eventClause eventAction;`
...
* **Statement** ([`CONSTRAINT`](/CONSTRAINT_statement/.md)): `CONSTRAINT [eventClause] constraintExpr [CHECKED [BY propertyId1, ..., propertyIdN]] MESSAGE messageExpr [PROPERTIES outExpr1, ..., outExprM];`
...
* **Operator** ([`AGGR`](/AGGR_operator/.md)): `AGGR [eventClause] aggrClass WHERE aggrExpr [NEW [newEventClause]] [DELETE [deleteEventClause]]`

## View logic — the same model, shown to a user or to another system[​](#view-logic--the-same-model-shown-to-a-user-or-to-another-system "Direct link to View logic — the same model, shown to a user or to another system")
...
* **Statement** ([`FORM`](/FORM_statement/.md)): `FORM name [caption] formOptions formBlock1 ... formBlockN;`
...
### Navigator and windows[​](#navigator-and-windows "Direct link to Navigator and windows")
...
## Physical model — how a project is built and how it runs[​](#physical-model--how-a-project-is-built-and-how-it-runs "Direct link to Physical model — how a project is built and how it runs")
...
* **Statement** ([module header](/Module_header/.md)): `MODULE name;` `[REQUIRE moduleName1, ..., moduleNameN;]` `[PRIORITY namespaceName1, ..., namespaceNameM;]` `[NAMESPACE namespaceName;]`

### System modules (standard library)[​](#system-modules-standard-library "Direct link to System modules (standard library)")
...
### Development and integration[​](#development-and-integration "Direct link to Development and integration")
...
### Execution — telling the platform how to store and process[​](#execution--telling-the-platform-how-to-store-and-process "Direct link to Execution — telling the platform how to store and process")
...
### Management — running the result[​](#management--running-the-result "Direct link to Management — running the result")

* Launch parameters and working settings, the user interface, the [security policy](/Security_policy/.md), the interpreter, the scheduler, backups, monitoring, logs, the profiler and the chat. No brief article of its own: retrieve these from the `paradigm` branch.
