...
9. To check whether a property is `NULL`, the assistant SHOULD use `IF NOT property(...)`.
   ...
   This applies to a property of any class, not only `BOOLEAN`: a condition holds when its value is not `NULL`, whatever that value is — `hasNotes(o) = IF notes(o) THEN TRUE` for a `STRING`, `isScheduled(o) = IF scheduledAt(o) THEN TRUE` for a `DATETIME`, `hasDepartment(e) = IF department(e) THEN TRUE` for an object-valued property, `NOT query() OR name(o) LIKE ('%' + query() + '%')` for a search filter that passes everything while the query is empty (`NOT` and `OR` do not propagate `NULL`, rule 3).
