...
### Selecting the component[​](#selecting-the-component "Direct link to Selecting the component")
...
### Reading the projection[​](#reading "Direct link to Reading the projection")
...
### Bucketing rows into cells[​](#bucketing "Direct link to Bucketing rows into cells")
...
### Placing an lsf child[​](#lsf-child "Direct link to Placing an lsf child")
...
### A live editor in every row[​](#a-live-editor-in-every-row "Direct link to A live editor in every row")
...
### Extending a container with lsf children[​](#extending-a-container-with-lsf-children "Direct link to Extending a container with lsf children")
...
* **It does not block the user.** The web client sends it as an asynchronous request, without blocking input and without the [busy indicator](/Interactive_view.md#busy) a synchronous request gets, so the user keeps working while the form is re-read.
...
#### The standard button[​](#standard-button "Direct link to The standard button")
...
A React component can also draw the window the forms open in, instead of the standard tabs. The [`WINDOW`](/WINDOW_statement.md) `System.forms` is given the component name, and the open forms become its data - and so is any other `FORMS` window the application declares, which gets the forms opened into it with `SHOW ... WINDOW windowName`:
...
### The application header[​](#application-header "Direct link to The application header")

A header with a search field that finds records is a form: it wants a live projection, a session to hold the query, ordering and paging, and a way to open what it finds. So it is built as a form and opened into a window of its own, a [`WINDOW ... FORMS`](/WINDOW_statement.md) above the forms area, with [`SHOW ... WINDOW windowName`](/SHOW_operator.md):
...
### Drawing an image[​](#image "Direct link to Drawing an image")
...
### Drawing a caption[​](#caption "Direct link to Drawing a caption")
