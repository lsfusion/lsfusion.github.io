...
Инструкция `+=` не создает новое свойство, а добавляет еще одну реализацию к уже объявленному [абстрактному свойству](/ru/Property_extension/.md).
...
* `param1, ..., paramN`
  ...
  Список [типизированных параметров](/ru/IDs/.md#paramid) добавляемой реализации. Он задает ее сигнатуру. Список может быть пустым. Количество параметров и их классы должны быть совместимы с сигнатурой абстрактного свойства. Эти параметры можно использовать в `implementationExpr` и, для формы `CASE`, в `conditionExpr`.

* `implementationExpr`

  [Выражение](/ru/Expression/.md) для реализации свойства. Его класс результата должен быть совместим с классом результата абстрактного свойства.
