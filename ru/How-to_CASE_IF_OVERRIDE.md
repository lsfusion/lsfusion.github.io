...
Прямолинейное условие `ord(dd) <= ord(d)` сравнивает друг с другом два многоветочных выбора: при компиляции разворачивается произведение их ветвей, поэтому с ростом числа гранулярностей и тяжести свойств-ветвей текст запроса растёт мультипликативно и может превысить максимальную длину запроса. Поветочное сравнение сравнивает каждую гранулярность только с самой собой: в смешанной паре один из операндов каждого сравнения равен `NULL`, и пара не учитывается — здесь это ровно требуемая семантика. Так переписать сравнение можно, только когда значения разных ветвей не должны сравниваться между собой; в противном случае храните составной номер, пометив свойство [`MATERIALIZED`](/ru/Materializations/.md).
