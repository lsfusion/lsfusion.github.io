...
Оператор *пользовательской формулы* позволяет создавать [свойство](/ru/Properties/.md), вычисляющее некоторую формулу на языке SQL. При этом можно задать одну реализацию, используемую для любого SQL-сервера, или различные реализации для разных SQL серверов.
