...
Значения, полученные на всех итерациях, агрегируются: если `initialExpr` и `stepExpr` принадлежат числовому классу, используется агрегация `SUM`; в остальных случаях (как правило, при классе `BOOLEAN`) используется агрегация `OR`. Подробное описание семантики и политик работы с циклами см. в разделе [Рекурсия (RECURSION)](/ru/Recursion_RECURSION/.md).
