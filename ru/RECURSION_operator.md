...
Значения, полученные на всех итерациях, агрегируются: если `initialExpr` и `stepExpr` имеют класс `BOOLEAN`, используется агрегация `OR`; в остальных случаях — `SUM`. Подробное описание семантики и политик работы с циклами см. в разделе [Рекурсия (RECURSION)](/ru/Recursion_RECURSION/.md).
