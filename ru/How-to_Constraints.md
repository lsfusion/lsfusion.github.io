...
Оба варианты идентичны с точки зрения выполнения. Если платформа не находит в ограничении никакого [оператора изменений](/ru/Change_operators_SET_CHANGED_etc/.md), то все выражение автоматически "оборачивается" в оператор `SET`.
...
Аналогично [**Примеру 1**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-1) и [**Примеру 2**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-2). Также для заказа заданы строки со ссылкой на книгу и указанием цены.
...
Аналогично [**Примеру 4**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-4). Введено понятие покупатель и возможность выбора книг, доступных ему.
...
Аналогично [**Примеру 2**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-2). Для заказа задан признак разрешения редактирования.
...
[Условие ограничения проверяется и на объектах, удаляемых в сессии](/ru/Constraints/.md): при удалении заказа его первичные свойства сбрасываются в `NULL`, поэтому `CHANGED(date(o))` истинно, а `NOT editable(o)` тоже истинно — свойство удалённого объекта даёт `NULL`, и отрицание `NULL` истинно. Без `o IS Order` такое ограничение неожиданно блокировало бы удаление любого заказа с заданной датой. Для удалённого заказа `o IS Order` даёт `NULL`, и ограничение не срабатывает. В [**Примере 2**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-2) защита не нужна: там условие `posted(o)` положительное, и для удалённого заказа оно само даёт `NULL`. Намеренный запрет удаления, наоборот, строится через `DROPPED`, как в [**Примере 3**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-3).
