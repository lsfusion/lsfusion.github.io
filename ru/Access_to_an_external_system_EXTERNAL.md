...
Платформа предоставляет возможность обращаться к внешним системам с использованием различных типов взаимодействия / протоколов, как внешний аналог [внутреннего вызова](/ru/Internal_call_INTERNAL/.md). Интерфейсом такого обращения является выполнение кода на языке / в парадигме внешней системы с заданными параметрами и, при необходимости, возврат некоторых значений в качестве *результатов*, записываемых в заданные свойства (без параметров). Предполагается, что все объекты параметров и результатов являются объектами [встроенных классов](/ru/Built-in_classes/.md).

## Типы взаимодействий / протоколы[​](#типы-взаимодействий--протоколы "Прямая ссылка на этот заголовок")

На данный момент в платформе поддерживаются следующие типы взаимодействий / внешних систем:

### HTTP - выполнение http-запроса Web-сервера[​](#http "Прямая ссылка на этот заголовок")
...
#### Результаты[​](#результаты "Прямая ссылка на этот заголовок")
...
Отметим, что обработка параметров и результатов http запроса во многом аналогична их обработке в [обращении из внешней системы](/ru/Access_from_an_external_system/.md) по протоколу HTTP (параметры при этом обрабатываются как результаты, и, наоборот, результаты обрабатываются как параметры)

### SQL - выполнение команды SQL-сервера.[​](#sql "Прямая ссылка на этот заголовок")
...
#### Параметры[​](#table "Прямая ссылка на этот заголовок")
...
#### Результаты[​](#результаты-1 "Прямая ссылка на этот заголовок")
...
### LSF - вызов действия другого lsFusion-сервера[​](#lsf---вызов-действия-другого-lsfusion-сервера "Прямая ссылка на этот заголовок")
...
Способ задания действия в этом типе взаимодействия полностью соответствует [способу задания](/ru/Access_from_an_external_system/.md#actiontype) действия при [обращении из внешней системы](/ru/Access_from_an_external_system/.md).

По умолчанию этот тип взаимодействия реализуется по протоколу HTTP с использованием соответствующих интерфейсов обращений [к](#http) и [из](/ru/Access_from_an_external_system/.md#http) внешней системы.

к сведению

Также для обращения к внешним системам можно использовать операторы [записи](/ru/Write_file_WRITE/.md) и [чтения](/ru/Read_file_READ/.md) файлов (в том случае, если интерфейсом такого взаимодействия является обмен файлами).

### TCP / UDP - передача байт через сокет[​](#tcp "Прямая ссылка на этот заголовок")
...
### DBF - запись строк в `.dbf`-файл[​](#dbf "Прямая ссылка на этот заголовок")
...
## Сохранение подключений между вызовами[​](#сохранение-подключений-между-вызовами "Прямая ссылка на этот заголовок")

При серии обращений к одной и той же конечной точке открытое подключение можно [переиспользовать](/ru/New_connection_NEWCONNECTION/.md) между вызовами вместо открытия нового на каждый. Это снижает накладные расходы на установление подключения и позволяет передавать между вызовами состояние, которое подключение хранит у себя — временные таблицы и переменные SQL-сессии, открытый TCP-сокет и буфер чтения, текущее положение в DBF-файле и т.п.

## Язык[​](#язык "Прямая ссылка на этот заголовок")

Для объявления действия, обращающегося к внешней системе, используется [оператор `EXTERNAL`](/ru/EXTERNAL_operator/.md). Для переиспользования внешнего подключения между несколькими вызовами в одном блоке используется [оператор `NEWCONNECTION`](/ru/NEWCONNECTION_operator/.md).
