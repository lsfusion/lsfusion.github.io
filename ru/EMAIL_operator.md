...
Оператор `EMAIL` - создание [действия](/ru/Actions/.md), [отправляющего почту](/ru/Send_mail_EMAIL/.md).
...
В опции `NAME` в `attachDescription` имена файлов задаются без расширений (точка `.` будет считаться частью имени файла). Расширение будет определено автоматически по аналогии с [оператором `WRITE`](/ru/WRITE_operator/.md). Если же опция `NAME` не задана, то имя вложения по умолчанию имеет вид `'attachmentK'`, где `K` - порядковый номер вложения.

### Параметры[​](#параметры "Прямая ссылка на этот заголовок")

* `fromExpr`

  [Выражение](/ru/Expression/.md), значение которого определяет адрес отправителя письма.
...
* `attachFilePropertyID`
  ...
  [Идентификатор свойства](/ru/IDs/.md#propertyid), которое определяет список файлов, прикрепляемых к письму. Свойство должно иметь ровно один параметр класса `INTEGER` и возвращать значение файлового класса.
