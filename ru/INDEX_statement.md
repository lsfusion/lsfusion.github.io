...
Инструкция `INDEX` создает новый [индекс](/ru/Indexes/.md).
...
Здесь `field1, ..., fieldN` - непустой список полей. Каждый его элемент может быть записан либо как [свойство](/ru/Properties/.md), хранящееся в этой таблице:

```
propertyId(param1, ..., paramN)
```

либо как [типизированный параметр](/ru/IDs/.md#paramid), указывающий на соответствующее ключевое поле:
...
* `propertyId`
  ...
  [Идентификатор свойства](/ru/IDs/.md#propertyid), которое должно храниться в таблице, для которой создается индекс.

* `param1, ..., paramN`

  Список параметров свойства. Каждый элемент списка является [типизированным параметром](/ru/IDs/.md#paramid).

  В `param1, ..., paramN` имена параметров должны быть различны.

* `param`

  [Типизированный параметр](/ru/IDs/.md#paramid), определяющий ключевое поле таблицы.
