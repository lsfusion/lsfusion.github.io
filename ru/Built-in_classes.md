...
*Встроенные классы* - это [классы](/ru/Classes/.md), экземплярами которых являются объекты примитивных типов данных, такие как целые числа, строки, и т.п.
...
Класс, образующий собственное семейство, несовместим с классами других семейств и не имеет с ними общего предка. В частности, класс `HTML` (в отличие от `HTMLTEXT`, входящего в семейство строк) несовместим со строковыми классами: [выбор](/ru/Selection_CASE_IF_MULTI_OVERRIDE_EXCLUSIVE/.md) с ветками классов `HTML` и `STRING` не имеет общего предка, поэтому такое свойство не может принимать значений, и сервер при старте выдаёт ошибку `property '...' is always NULL`. Сложение (`+`) значения класса `HTML` со строкой при этом не является ошибкой, но возвращает обычную строку: результат теряет класс `HTML`, и разметка при отображении будет экранирована.

## Общий предок[​](#commonparentclass "Прямая ссылка на этот заголовок")

В соответствии с описанным механизмом наследования, общий предок двух встроенных классов (например для операции [выбора](/ru/Selection_CASE_IF_MULTI_OVERRIDE_EXCLUSIVE/.md)) определяется следующим образом:

### Строки[​](#строки "Прямая ссылка на этот заголовок")
...
### Числа[​](#числа "Прямая ссылка на этот заголовок")
...
### Файлы конкретного типа[​](#файлы-конкретного-типа "Прямая ссылка на этот заголовок")
...
### Ссылки на файлы конкретного типа[​](#ссылки-на-файлы-конкретного-типа "Прямая ссылка на этот заголовок")
...
## Значение по умолчанию[​](#defaultvalue "Прямая ссылка на этот заголовок")

В некоторых случаях для встроенного класса необходимо использовать некоторое значение, которое будет заведомо отличаться от `NULL` (например, в условии импорта при [импорте данных](/ru/Data_import_IMPORT/.md)). Это значение будем называть *значением по умолчанию*, и определяется оно следующим образом:
...
## Расширения файлов конкретного типа[​](#extension "Прямая ссылка на этот заголовок")

При преобразовании файлов конкретного типа (`JSONFILE`, `XMLFILE`, ...) к файлу динамического типа (`FILE`, `NAMEDFILE`), как явном, так и неявном (например при [импорте данных](/ru/Data_import_IMPORT/.md) без указания формата или при [взаимодействии с внешними системами](/ru/Access_to_an_external_system_EXTERNAL/.md)) расширение результирующего файла определяется следующим образом:
...
## Порядок определения результирующего свойства при [обращении из внешней системы](/ru/Access_from_an_external_system/.md#httpresult)[​](#export "Прямая ссылка на этот заголовок")
...
| [Пользовательские классы](/ru/User_classes/.md)                                                                                                                                | `exportObject`                                                                                                                                                                                                                                        |
