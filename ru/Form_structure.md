...
Структура [формы](/ru/Forms/.md) - это совокупность *объектов*, [свойств](/ru/Properties/.md), [действий](/ru/Actions/.md) и связей между ними.

### Объекты[​](#objects "Прямая ссылка на этот заголовок")

При создании формы первоначально необходимо определить, какие объекты она будет отображать. Для каждого объекта формы нужно задать его [класс](/ru/Classes/.md).
...
#### Текущий объект[​](#currentObject "Прямая ссылка на этот заголовок")

Каждый объект на форме в любой момент времени имеет *текущее значение*. Его изменение происходит либо в результате [соответствующих действий](/ru/Interactive_view/.md#objects) пользователя в [интерактивном](/ru/Interactive_view/.md) представлении, либо "виртуально" в процессе чтения данных в [статичном](/ru/Static_view/.md) представлении.

### Свойства и действия[​](#properties "Прямая ссылка на этот заголовок")
...
Добавление действий актуально только для [интерактивного](/ru/Interactive_view/.md) представления. В [статичном](/ru/Static_view/.md) представлении добавленные действия игнорируются.
...
#### Группа отображения[​](#drawgroup "Прямая ссылка на этот заголовок")

Каждое свойство отображается ровно в одной из групп объектов на форме (будем называть ее *группой отображения* этого свойства). По умолчанию группой отображения является группа, последняя для множества объектов, которые передаются на вход этому свойству. При необходимости, разработчик может задать группу отображения явно (с определенными [ограничениями](/ru/Structured_view/.md#drawgroup) при использовании в структурированном представлении)

#### Группы-в-колонки[​](#groupcolumns "Прямая ссылка на этот заголовок")
...
#### Группы свойств[​](#propertygroup "Прямая ссылка на этот заголовок")

Свойства на форме можно объединять в [группы](/ru/Groups_of_properties_and_actions/.md), которые, в свою очередь, используются в интерактивном ([дизайне по умолчанию](/ru/Form_design/.md#defaultDesign)) и [иерархическом](/ru/Structured_view/.md#hierarchy) представлениях формы. По умолчанию, привязка свойства к группе глобальна (то есть задается для свойства для всех форм сразу), однако, при необходимости, для отдельных форм эту привязку можно переопределить.

#### Настройки по умолчанию[​](#настройки-по-умолчанию "Прямая ссылка на этот заголовок")

Для свойств на форме существует большое количество настроек отображения в различных [представлениях](/ru/Form_views/.md), абсолютное большинство из которых можно задавать не только непосредственно для свойства на форме, но и для самого добавляемого свойства (при его создании). Эти настройки будут настройками по умолчанию, то есть если настройка не задается явно для конкретного свойства на форме, то используется настройка самого свойства. При этом, эти настройки по умолчанию "наследуются" при использовании операторов [композиции](/ru/Composition_JOIN/.md), [выбора в одиночной форме](/ru/Selection_CASE_IF_MULTI_OVERRIDE_EXCLUSIVE/.md#single) без альтернативного результата и [предыдущего значения](/ru/Previous_value_PREV/.md) (то есть, если скажем для свойства `f(a)` задана ширина по умолчанию `10`, то и свойство `g(a) = f(a) IF h(a)`, также по умолчанию будет иметь ширину `10`).

### Фильтры[​](#filters "Прямая ссылка на этот заголовок")
...
### Сортировки[​](#sort "Прямая ссылка на этот заголовок")
...
Для создания новой формы и описания ее структуры предназначена [инструкция `FORM`](/ru/FORM_statement/.md).
