...
* [Правила: импорт данных (IMPORT)](#%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D0%B8%D0%BC%D0%BF%D0%BE%D1%80%D1%82-%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85-import)
* [Правила: экспорт данных (EXPORT)](#%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D1%8D%D0%BA%D1%81%D0%BF%D0%BE%D1%80%D1%82-%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85-export)

## Правила: импорт данных (IMPORT)[​](#правила-импорт-данных-import "Прямая ссылка на этот заголовок")
...
6. Для импорта через форму ассистент ОБЯЗАН объявить выделенную форму импорта до использования.
   ...
   * `EXTID`, `FORMEXTID`, группы и `ATTR` только там, где этого требует внешняя схема — массив на корневом уровне `JSON` как раз такое место (правило 7)

   Ассистент ОБЯЗАН помнить, что импорт в форму отменяет ожидающие изменения импортируемых свойств формы в текущей сессии.

7. Для массива на корневом уровне `JSON` ассистент ОБЯЗАН задать группе объектов формы импорта имя экспорта / импорта `value` (`OBJECTS receipts = INTEGER EXTID 'value'`): такой файл платформа читает как `{ "value" : [ ... ] }` (см. [предопределенное значение](/ru/Structured_view.md#value)).

   Группа объектов, чьему [имени экспорта / импорта](/ru/Structured_view.md#extid) не соответствует ни один ключ файла, импортирует ноль записей без ошибки и без предупреждения, поэтому новую форму импорта ассистент ОБЯЗАН проверить на непустом примере.

8. После импорта формы ассистент НЕ ДОЛЖЕН итерировать по `imported[INTEGER]`, если на форме нет фильтра с ним (`FILTERS imported(receipts)`): импорт формы записывает данные только в [свойства и фильтры формы](/ru/In_a_structured_view_EXPORT_IMPORT.md#importForm). Без фильтра эту роль играет промежуточное свойство, которое заполнено всегда.
...
9. Ассистент ОБЯЗАН явно выбирать опции формата, когда от них зависит внешний контракт:
...
10. Ассистент ОБЯЗАН валидировать ссылочные бизнес-ключи до создания или обновления постоянных объектов.
...
11. Ассистенту СЛЕДУЕТ разделять сырой импорт и доменное разрешение:
...
12. Для запускаемых пользователем пакетных импортов и внешних интеграций ассистенту СЛЕДУЕТ изолировать сохранение в `NEWSESSION` и СЛЕДУЕТ выполнять `APPLY;` после доменных записей одного импорта.
...
13. Ассистент НЕ ДОЛЖЕН частично сохранять неудавшийся импорт молча. Для ошибок, которые ассистент обнаруживает сам (отсутствующие ссылки, некорректные данные, валидация до `APPLY`), ему СЛЕДУЕТ использовать `MESSAGE`, `RETURN`, `throwException` или явный флаг неудачи, согласованно с вызывающей стороной:

    * интерактивный импорт -> `MESSAGE`
    * API или фоновая интеграция -> исключение или явное состояние неудачи

14. Для импортов-синхронизаций «создать-или-обновить» ассистент ОБЯЗАН разделять создание объектов и обновление свойств.
...
15. Если `LOCAL`-свойства промежуточного хранения используются только в одном действии импорта, ассистент ОБЯЗАН объявлять их внутри этого действия.
...
## Правила: экспорт данных (EXPORT)[​](#правила-экспорт-данных-export "Прямая ссылка на этот заголовок")

### Выбор источника экспорта[​](#выбор-источника-экспорта "Прямая ссылка на этот заголовок")

Данные экспортируются [оператором `EXPORT`](/ru/EXPORT_operator.md).
...
### Явное указание того, что влияет на результат[​](#явное-указание-того-что-влияет-на-результат "Прямая ссылка на этот заголовок")
...
### Опции формата[​](#опции-формата "Прямая ссылка на этот заголовок")
...
### Приемник результата[​](#приемник-результата "Прямая ссылка на этот заголовок")
...
### Передача результата[​](#передача-результата "Прямая ссылка на этот заголовок")
