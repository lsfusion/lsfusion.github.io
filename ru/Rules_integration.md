...
6. Для массива на корневом уровне `JSON` ассистент ОБЯЗАН задать группе объектов формы импорта имя экспорта / импорта `value` (`OBJECTS receipts = INTEGER EXTID 'value'`): такой файл платформа читает как `{ "value" : [ ... ] }` (см. [предопределенное значение](/ru/Structured_view.md#value)).
...
7. После импорта формы ассистент НЕ ДОЛЖЕН итерировать по `imported[INTEGER]`, если на форме нет фильтра с ним (`FILTERS imported(receipts)`): импорт формы записывает данные только в [свойства и фильтры формы](/ru/In_a_structured_view_EXPORT_IMPORT.md#importForm). Без фильтра эту роль играет промежуточное свойство, которое заполнено всегда.
