...
Оператор *изменения класса* создает [действие](/ru/Actions/.md), которое присваивает заданный [класс](/ru/Classes/.md) всем объектам, удовлетворяющим условию. Условие — выражение от тех же аргументов, что и выражение объекта; его можно не задавать, в этом случае оно считается выполняющимся всегда. Целевой класс должен быть конкретным [пользовательским классом](/ru/User_classes/.md).
...
Для объявления действия, изменяющего класс объектов, используется [оператор `CHANGECLASS`](/ru/CHANGECLASS_operator/.md); для удаления — [оператор `DELETE`](/ru/DELETE_operator/.md).
