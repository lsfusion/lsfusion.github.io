...
* `rootExpr`
  ...
  [Выражение](/ru/Expression/.md) строкового класса, задающее имя элемента, считаемого корнем при импорте. В исходном файле выполняется рекурсивный поиск первого элемента с таким именем; от найденного узла начинается обработка вложенных записей. Применяется только для импорта **JSON** и **XML**.
