...
`System` — корневой [системный модуль](/ru/System_modules.md) платформы. Подключается автоматически: любой другой модуль зависит от него без явного `REQUIRE`. Модуль объявляет корневые [классы](/ru/User_classes.md), стандартные [группы](/ru/Groups_of_properties_and_actions.md) свойств и действий, поверхность управления [сессией изменений](/ru/Change_sessions.md), контекст входящего HTTP-запроса, действия жизненного цикла формы, свойства идентичности приложения и локальные буферы, в которые пишут операторы экспорта, импорта и запроса значений.

### Корневые классы[​](#корневые-классы "Прямая ссылка на этот заголовок")
...
### Стандартные группы[​](#стандартные-группы "Прямая ссылка на этот заголовок")
...
### Метаданные объектов[​](#метаданные-объектов "Прямая ссылка на этот заголовок")
...
### Управление сессией изменений[​](#управление-сессией-изменений "Прямая ссылка на этот заголовок")
...
### Контекст HTTP-запроса[​](#контекст-http-запроса "Прямая ссылка на этот заголовок")
...
### Состояние пойманного исключения[​](#состояние-пойманного-исключения "Прямая ссылка на этот заголовок")
...
### Открытие файлов и ссылок[​](#открытие-файлов-и-ссылок "Прямая ссылка на этот заголовок")
...
### Жизненный цикл формы[​](#жизненный-цикл-формы "Прямая ссылка на этот заголовок")
...
### Признаки контекста формы[​](#признаки-контекста-формы "Прямая ссылка на этот заголовок")
...
### Полиморфные edit и delete[​](#полиморфные-edit-и-delete "Прямая ссылка на этот заголовок")
...
### Локальные буферы[​](#локальные-буферы "Прямая ссылка на этот заголовок")
...
* **Импорт**: `importFile[]` — файл, из которого по умолчанию импортируется форма. `imported[INTEGER]` и `importedString[STRING[10]]` — признак того, что соответствующая «строка» прочитана из файла: `imported[INTEGER]` по умолчанию служит условием [импорта данных](/ru/Data_import_IMPORT.md), а при [импорте формы](/ru/In_a_structured_view_EXPORT_IMPORT.md#importForm) заполняется, только если используется в ее фильтре. `inputList[INTEGER]` / `displayInputList[INTEGER]` — буфер списка значений и подписей. `readFile[]`, `readDialogPath[]`, `showResult[]` — результаты чтения файла, выбора пути и вывода сообщения.

### Идентичность приложения[​](#идентичность-приложения "Прямая ссылка на этот заголовок")
...
### Оформление по умолчанию[​](#оформление-по-умолчанию "Прямая ссылка на этот заголовок")
...
### Среда исполнения и проверки[​](#среда-исполнения-и-проверки "Прямая ссылка на этот заголовок")
...
### Работа с файлами[​](#работа-с-файлами "Прямая ссылка на этот заголовок")
...
* [Заголовок модуля](/ru/Module_header.md) — синтаксис `MODULE` / `REQUIRE`; модуль `System` подключается автоматически.
* [Оператор `APPLY`](/ru/APPLY_operator.md) — лежит в основе действий управления сессией.
* [Оператор `CANCEL`](/ru/CANCEL_operator.md) — отмена изменений сессии.
* [Оператор `EXPORT`](/ru/EXPORT_operator.md) — пишет в локальные буферы `export…`.
* [Оператор `IMPORT`](/ru/IMPORT_operator.md) — наполняет `imported`, `importedString` и связанные локальные свойства.
* [Инструкция `+=`](/ru/plus_equals_statement.md) — добавляет реализацию проекта для оконного свойства-класса.
* [Инструкция `WINDOW`](/ru/WINDOW_statement.md) — объявляет собственное окно проекта с его CSS-классом.

### Связано[​](#связано "Прямая ссылка на этот заголовок")

* [`System modules`](/ru/System_modules.md) — общий перечень модулей платформы.
* [`Time`](/ru/System_Time.md) — отдельный модуль времени, подключающийся через `REQUIRE Time`.
* [`Utils`](/ru/System_Utils.md) — сборник вспомогательных свойств.
* [`Сессия изменений`](/ru/Change_sessions.md) — концепция сессии и её фиксации.
* [`Формы`](/ru/Forms.md) — что такое форма и как её жизненный цикл связан с системными действиями.
