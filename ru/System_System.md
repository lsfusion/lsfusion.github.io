...
Модуль фиксирует основные [окна](/ru/Form_views/.md): `logo`, `root`, `system` (верхняя горизонтальная панель), `toolbar` (левая вертикальная), `forms` (центральная область), `log` (правая полоса для уведомлений). Под каждое окно есть абстрактный CSS-класс (`logoWindowClass[]`, …) для кастомизации.
