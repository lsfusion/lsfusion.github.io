...
Наиболее простым способом организации взаимодействия React приложения с приложением на базе **lsFusion** является [общение через HTTP-протокол](/ru/Access_from_an_external_system/.md) посредством JSON API. Для выгрузки списка объектов по заданному условию удобнее всего использовать интерфейс, который находится по Url'у eval/action. На него в BODY можно передать программный код lsFusion, который будет выполнен. При необходимости вернуть данные по запросу необходимо использовать [оператор `EXPORT`](/ru/EXPORT_operator/.md). По умолчанию, он возвращает данные в формате JSON, которые затем легко обрабатываются при помощи JavaScript.
...
Для наглядности реализуем простую форму из примера [Турнирная таблица](/ru/Score_table/.md) (без возможности редактирования).
...
Рассмотрим немного более сложный случай, когда нужно делать параметризованные запросы, в зависимости от выбранных пользователем данных. В качестве логики backend'а возьмем пример [Управление материальными потоками](/ru/Materials_management/.md).
...
Как и в примерах выше, названия параметров не важны, а важен их порядок. [Оператор `APPLY`](/ru/APPLY_operator/.md) сохраняет изменения в базу данных. Без него данные сохранены не будут и сбросятся по завершению выполнения запрос.
...
Для разбора JSON используем специальный [оператор `IMPORT`](/ru/IMPORT_operator/.md). Он предполагает, что ему на вход будет плоский JSON в виде массива объектов с числовыми полями `item`, `quantity`, `price` и `sum`. Для каждого объекта в нем будет создан свой `ShipmentDetail`. Затем он привязывается к соответствующему объекту `s` класса `Shipment`. После этого проставляется товар, у которого внутренний код совпадает с переданным, и остальные свойства. Разбор более сложных JSON можно найти в примерах [How-to: Импорт данных](/ru/How-to_Data_import/.md) и [How-to: Взаимодействие через HTTP-протокол](/ru/How-to_Interaction_via_HTTP_protocol/.md).

При сохранении изменений может нарушиться [ограничение](/ru/Constraints/.md). В этом случае изменения не будут сохранены в базу данных (при этом они останутся в [сессии изменений](/ru/Change_sessions/.md)). При этом в свойство `canceled` будет записано `TRUE`, а в свойство `applyMessage` сообщение ограничения. Для того, чтобы обработать эту ситуацию будем использовать следующий код на платформе:
...
[suspicious-shockley-r16kv](https://codesandbox.io/embed/suspicious-shockley-r16kv?fontsize=14)

### Пример 5[​](#пример-5 "Прямая ссылка на этот заголовок")

По аналогии с [**Примером 1**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-1) реализуем форму Турнирная таблица с возможностью редактирования при помощи [Form API](/ru/Access_from_an_external_system/.md#form). Для его использования требуется подключить библиотеку [@lsfusion/core](https://www.npmjs.com/package/@lsfusion/core).
...
Такая же схема используется для изменений свойств, которые не являются первичными, а являются простой [композицией](/ru/Composition_JOIN/.md) от объектных свойств:
