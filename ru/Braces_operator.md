...
[Параметры](/ru/IDs.md#paramid), введённые локально присваиванием, `FOR` или `NEW`, видны только внутри этого оператора; то же имя в следующем операторе вводит независимый параметр. Параметры объемлющего действия или оператора остаются доступны в каждом операторе последовательности.
