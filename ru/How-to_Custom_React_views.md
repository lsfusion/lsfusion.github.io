...
Контейнер [`DESIGN`](/ru/DESIGN_statement.md) может отрисовываться компонентом React вместо стандартной раскладки. Компонент получает проекцию состояния формы и сам рисует всё поддерево контейнера.
...
### Выбор компонента[​](#selecting-the-component "Прямая ссылка на этот заголовок")
...
### Чтение проекции[​](#reading "Прямая ссылка на этот заголовок")
...
### Отрисовка строк[​](#rendering-rows "Прямая ссылка на этот заголовок")
...
### Раскладка строк по ячейкам[​](#bucketing "Прямая ссылка на этот заголовок")
...
### Размещение lsf-компонента[​](#lsf-child "Прямая ссылка на этот заголовок")
...
### Живой редактор в каждой строке[​](#живой-редактор-в-каждой-строке "Прямая ссылка на этот заголовок")
...
### Расширение контейнера с lsf-компонентами[​](#расширение-контейнера-с-lsf-компонентами "Прямая ссылка на этот заголовок")
...
### Выбор между компонентом React и HTML-шаблоном[​](#выбор-между-компонентом-react-и-html-шаблоном "Прямая ссылка на этот заголовок")
...
### Интерактивность[​](#интерактивность "Прямая ссылка на этот заголовок")

Чтобы читать и изменять состояние формы из компонента — выбирать строку, изменять свойство, вызывать действия — используется `props.controller`. Его методы описаны в [How-to: Контроллер пользовательского представления](/ru/How-to_Custom_view_controller.md).

Компонент, который рисует собственное поле, также сообщает форме, что пользователь в нём редактирует, чтобы во время набора `ENTER` не переводил фокус на следующий компонент, а `ESCAPE` не закрывал окно — см. [Объявление редактирования](/ru/How-to_Custom_view_controller.md#editing). Компоненту, который своих полей не рисует, объявлять нечего.

### Живые данные[​](#live-data "Прямая ссылка на этот заголовок")

Данные, которые меняются, пока форма открыта, — котировки, очередь, монитор — обновляет сама форма, а не таймер в компоненте. [Событие `SCHEDULE`](/ru/Form_events.md) формы выполняет действие раз в заданное число секунд, а изменившееся платформа доставляет через `props.data`, как любое другое изменение:
...
* **Не блокирует пользователя.** Веб-клиент отправляет его асинхронным запросом, не блокируя ввод и без [индикатора ожидания](/ru/Interactive_view.md#busy), который получает синхронный запрос, так что пользователь продолжает работать, пока форма перечитывается.
...
### Окно навигатора[​](#navigator-window "Прямая ссылка на этот заголовок")

Компонент React может рисовать и окно навигатора — само меню — вместо стандартной панели. Имя компонента задаётся [`WINDOW`](/ru/WINDOW_statement.md), а данными компонента становятся элементы, помещённые в это окно:
...
`props.data` — собственные элементы этого окна по [каноническому имени](/ru/Naming.md#canonicalname); канонические имена содержат точки, поэтому они являются ключами отображения, а не полями:
...
Элемент, объявленный [`LSF`](/ru/NAVIGATOR_statement.md), рисует платформа, поэтому то, что она рисует, не проецируется: в его записи остаются `name`, `caption`, `image`, `children` и `lsf: true`, и больше ничего.

Окно получает те элементы, за которые отвечает: элемент, ушедший в другое окно, и поддерево, отсечённое потому, что его родитель не выбран, отсутствуют — по тому же правилу, по которому определяется содержимое стандартной панели. Всё, что это правило оставило, проекция сохраняет, скрытое оно или нет. `props.controller` — контроллер навигатора, чей [`activate`](/ru/How-to_Custom_view_controller.md#navigator-controller) делает то же, что нажатие на элемент: выбирает папку либо выполняет действие.
...
#### Стандартная кнопка[​](#standard-button "Прямая ссылка на этот заголовок")
...
Элемент должен быть объявлен `LSF` в [`NAVIGATOR`](/ru/NAVIGATOR_statement.md) — переход обратно объявляется на стороне навигатора так же, как `lsf = TRUE` объявляет его на форме:
...
#### HTML-шаблон вместо компонента[​](#navigator-template "Прямая ссылка на этот заголовок")

Окну, меню которого нужна только своя разметка, вместо имени компонента задаётся HTML-шаблон — [тот же](/ru/WINDOW_statement.md), который принимает контейнер `custom`, где место `<Lsf:name>` принимает стандартную кнопку элемента. Имя разрешается так же, как любое другое имя в модуле, поэтому писать его полностью не нужно, а место пишется открытым — закрыть его нельзя:
...
### Окно форм[​](#forms-window "Прямая ссылка на этот заголовок")

Компонент React может рисовать и окно, в котором открываются формы, вместо стандартных вкладок. Окну [`WINDOW`](/ru/WINDOW_statement.md) `System.forms` задаётся имя компонента, а его данными становятся открытые формы - и так же любому другому окну `FORMS`, которое объявило приложение: ему передаются формы, открытые в него оператором `SHOW ... WINDOW windowName`:
...
| `canonicalName`    | [Каноническое имя](/ru/Naming.md#canonicalname) формы — для компонента, который рисует конкретную форму по-своему                                                                  |
...
### Шапка приложения[​](#application-header "Прямая ссылка на этот заголовок")

Шапка с полем поиска, которое находит записи, - это форма: ей нужны живая проекция, сессия, в которой живёт запрос, сортировка и постраничность, и способ открыть найденное. Поэтому она строится как форма и открывается в собственное окно - [`WINDOW ... FORMS`](/ru/WINDOW_statement.md) над областью форм - оператором [`SHOW ... WINDOW windowName`](/ru/SHOW_operator.md):
...
Чего маршрут через форму не даёт - свежести между соединениями: форма не видит того, что закоммитило другое соединение, пока не перечитает данные, поэтому шапка, которая должна это показывать, добавляет форме [`EVENTS ON SCHEDULE PERIOD n formRefresh()`](/ru/Interactive_view.md). И шапку рисует только десктопный веб-клиент: мобильный веб-клиент и десктоп-клиент рисуют одно окно `System.forms` и открывают шапку там, закладкой, поэтому приложение, которое их обслуживает, оставляет дизайн шапки пригодным и как обычной формы.

### Окно сообщений[​](#log-window "Прямая ссылка на этот заголовок")

Компонент React может рисовать и окно, в котором появляются сообщения пользователю, вместо стандартной панели. Окну [`WINDOW`](/ru/WINDOW_statement.md) `System.log` задаётся имя компонента, а его данными становятся выведенные сообщения:
...
### Отрисовка изображения[​](#image "Прямая ссылка на этот заголовок")
...
### Отрисовка заголовка[​](#caption "Прямая ссылка на этот заголовок")
