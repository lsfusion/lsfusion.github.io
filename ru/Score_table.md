...
Для этой цели создаем класс `GameResult` и добавляем ему три [статических объекта](/ru/Static_objects/.md), которые задаются с помощью выражений, заданных в фигурных скобках `{ }`. При этом значения `win`, `winOT`, `winSO` и `П`, `ПО`, `ПБ` будут доступны через системные свойства `name` и `caption`, соответственно.
