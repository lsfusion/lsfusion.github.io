...
Помимо рассмотренных выше методов отрисовки, *controller* позволяет клиентскому JS обращаться к серверу и получать результат в виде `Promise`. Тот же объект передается первым аргументом в JavaScript-функцию, заданную в действии [`INTERNAL CLIENT`](/ru/INTERNAL_operator/.md).
