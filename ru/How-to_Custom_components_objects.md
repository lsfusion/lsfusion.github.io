...
Определение «значение или строка» в `changeProperty`, формат значений и правила уточнения имени — те же, что у одноименного метода [контроллера формы](/ru/How-to_Custom_view_controller.md); свойство, не являющееся колонкой этой группы, `changeProperty` передает контроллеру формы, который разрешает его в пределах всей формы. `changeProperties` применяет несколько изменений одним запросом — например, представление диаграммы Ганта при перетаскивании полосы задачи изменяет обе даты сразу:
