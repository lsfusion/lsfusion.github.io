...
В *list* передается не весь набор объектов, а только считанная страница: для группы объектов с видом представления `CUSTOM` ее размер по умолчанию — 1000 объектов. Чтобы представление получало все объекты группы, задайте в блоке [`OBJECTS`](/ru/Object_blocks.md) опцию `PAGESIZE 0` (читать все объекты).
...
После удаления старых элементов для каждого объекта из массива *list* создается свой *div* *card*, в который помещаются нужные элементы отображения каждого свойства. Названия полей объектов соответствуют названию свойств на форме. Значения свойств преобразуются в значения JS так же, как в строках [React-представления](/ru/How-to_Custom_React_views.md): например, значения классов даты и времени передаются как `Date`, а `JSON` — как разобранный объект. Свойство, значением которого является объект, приходит числовым id этого объекта — тем же, которым объект передаётся в [запись FK и вызовы сервера](/ru/How-to_Custom_view_controller.md). При помощи метода *isCurrent* определяется, какой объект из списка является текущим.
...
Определение «значение или строка» в `changeProperty`, формат значений, правила уточнения имени и правило для действия — те же, что у одноименного метода [контроллера формы](/ru/How-to_Custom_view_controller.md); свойство, не являющееся колонкой этой группы, `changeProperty` передает контроллеру формы, который разрешает его в пределах всей формы. `changeProperties` применяет несколько изменений одним запросом — например, представление диаграммы Ганта при перетаскивании полосы задачи изменяет обе даты сразу:
...
Пользовательское представление объекта обращается к серверу через [контроллер формы](/ru/How-to_Custom_view_controller.md): он доступен как `controller.form` из локального контроллера представления, как `props.controller` в [React-представлении](/ru/How-to_Custom_React_views.md) либо как контроллер, передаваемый в функцию [`INTERNAL CLIENT`](/ru/INTERNAL_operator.md) (последним аргументом, после преобразованных параметров вызова). Его методы `exec` / `eval` / `evalAction` / `change` выполняются на сервере и возвращают `Promise`; их сигнатуры — в разделе [Вызов сервера](/ru/How-to_Custom_view_controller.md#calling-the-server). Далее в этом разделе — как ведут себя эти серверные вызовы: преобразование результата, сессии и авторизационная проверка.
