...
`Icon` — [системный модуль](/ru/System_modules/.md), хранящий каталог иконок интерфейса (Font Awesome и Bootstrap Icons) и подбирающий лучшую иконку под текстовый запрос через полнотекстовый поиск. Подключается через `REQUIRE Icon` (который, в свою очередь, делает `REQUIRE Utils, Reflection, SystemEvents`).

Свойства поиска и ранжирования опираются на примитивы `toTsVector` / `toTsQuery` / `tsRank` из модуля [`Utils`](/ru/System_Utils/.md) (см. его раздел «Полнотекстовый поиск»).

### Каталог иконок[​](#каталог-иконок "Прямая ссылка на этот заголовок")
...
### Полнотекстовый поиск и ранжирование[​](#полнотекстовый-поиск-и-ранжирование "Прямая ссылка на этот заголовок")
...
### Поиск лучшей иконки[​](#поиск-лучшей-иконки "Прямая ссылка на этот заголовок")
...
### Импорт иконок[​](#импорт-иконок "Прямая ссылка на этот заголовок")
...
### Свойства отображения[​](#свойства-отображения "Прямая ссылка на этот заголовок")
...
* [Оператор `MATCH`](/ru/MATCH_operator/.md) — полнотекстовое совпадение, по которому отбираются иконки-кандидаты под запрос.
* [Оператор `FORMULA`](/ru/FORMULA_operator/.md) — синтаксис, через который объявлены свойства ранжирования и разбиения строк.

### Связано[​](#связано "Прямая ссылка на этот заголовок")

* [`System modules`](/ru/System_modules/.md) — общий список модулей платформы.
* [`Utils`](/ru/System_Utils/.md) — его раздел «Полнотекстовый поиск» описывает примитивы `toTsVector` / `toTsQuery` / `tsRank`, на которых построен этот модуль.
* [`Reflection`](/ru/System_Reflection/.md) — модуль метаданных, подключаемый вместе с `Icon`.
* [`SystemEvents`](/ru/System_SystemEvents/.md) — модуль событий жизненного цикла сервера, подключаемый вместе с `Icon`.
