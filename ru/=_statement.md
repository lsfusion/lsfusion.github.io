...
Инструкция `=` - создание нового [свойства](/ru/Properties/.md).
...
Инструкция `=` объявляет новое свойство и добавляет его в текущий [модуль](/ru/Modules/.md).

При создании свойства инструкция имеет две основные формы:

* Первая форма описывает и создает свойство с помощью [выражения](/ru/Expression/.md). Класс возвращаемого значения и классы аргументов определяются по выражению.
* Вторая форма описывает свойство с помощью одного из [контекстно-независимых](/ru/Property_operators/.md#contextindependent) [операторов-свойств](/ru/Property_operators/.md), которые не могут быть частью выражения. Такой оператор сам несет в себе классы аргументов и возвращаемого значения.
...
* `name`
  ...
  Имя свойства. [Простой идентификатор](/ru/IDs/.md#id).

* `caption`

  Заголовок свойства. [Строковый литерал](/ru/Literals/.md#strliteral). Если заголовок не задан, то заголовком свойства будет являться его имя.

* `param1, ..., paramN`

  Список параметров. Каждый из них задается [типизированным параметром](/ru/IDs/.md#paramid). Список может быть пустым. Эти параметры далее могут быть использованы в выражении описывающем создаваемое свойство (а также в некоторых опциях).
...
* `expression`
  ...
  [Выражение](/ru/Expression/.md), описывающее и создающее свойство.

* `contextIndependentOperator`

  [Контекстно-независимый оператор-свойство](/ru/Property_operators/.md#contextindependent), описывающий и создающий свойство. Поскольку такой оператор сам задает классы своих аргументов, заданные в этой инструкции параметры (если они есть) лишь дают этим аргументам имена.

* `options`

  [Опции свойства](/ru/Property_options/.md).

* `returnClass`

  [Идентификатор класса](/ru/IDs/.md#classid) возвращаемого значения свойства.
