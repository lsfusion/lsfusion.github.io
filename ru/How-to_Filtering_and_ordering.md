...
Опция `FILTER` объявляет [пользовательский фильтр](/ru/Interactive_view.md#userfilters) для свойства покупателя. При открытии формы строка его условия уже показана с пустым значением и ничего не отфильтровывает; как только пользователь вводит покупателя, остаются только его заказы. Удалить строку пользователь не может - сброс фильтров лишь очищает её значение, - но, в отличие от локального свойства из [**Примера 3**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-3), при показанных элементах управления фильтрами пользователь может изменить сравнение, а свойство вне формы не нужно.
