...
а *классовая форма* открывает [форму выбора или редактирования](/ru/Interactive_view/.md#edtClass) класса (указывается ровно одно из `LIST` или `EDIT`) с одним неявным объектом, к которому `objSpec` прикрепляется напрямую:
