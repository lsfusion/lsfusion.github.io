...
Техника [расширения](/ru/Extensions/.md) [свойств](/ru/Properties/.md) позволяет объявить *абстрактное* свойство в одном [модуле](/ru/Modules/.md), а определять его реализации в других модулях. Это отложенное построение свойства, вычисляемого как соответствующий [оператор выбора](/ru/Selection_CASE_IF_MULTI_OVERRIDE_EXCLUSIVE/.md): базовый модуль задает вид будущего оператора и требования к его реализациям, а остальные модули постепенно добавляют отдельные реализации.
...
Так же как и [для оператора выбора](/ru/Selection_CASE_IF_MULTI_OVERRIDE_EXCLUSIVE/.md#poly), для абстрактного свойства существует *полиморфная форма*, когда условие выбора и соответствующий ему результат задаются одним свойством. Соответственно, как и в операторе выбора, условием может являться либо [принадлежность сигнатуре](/ru/Property_signature_ISCLASS/.md) этого свойства, либо само это свойство.

### Взаимоисключаемость условий[​](#exclusive "Прямая ссылка на этот заголовок")

Как и [для оператора выбора](/ru/Selection_CASE_IF_MULTI_OVERRIDE_EXCLUSIVE/.md#exclusive), для абстрактного свойства можно указать, что все его условия должны быть *взаимоисключающими*. В этом режиме для каждого набора аргументов должна существовать не более чем одна подходящая реализация.

### Язык[​](#язык "Прямая ссылка на этот заголовок")

В этой технике используются две языковые конструкции: [оператор `ABSTRACT`](/ru/ABSTRACT_operator/.md) для объявления абстрактного свойства и [инструкция `+=`](/ru/plus_equals_statement/.md) для добавления к нему реализации.
