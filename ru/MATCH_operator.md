...
Оператор `MATCH` создает [свойство](/ru/Properties/.md), реализующее [полнотекстовый поиск](/ru/Comparison_operators_=_etc/.md). Для сравнения строки с шаблоном используется отдельный [оператор `LIKE`](/ru/LIKE_operator/.md).
...
Оператор `MATCH` сравнивает `searchExpr` с поисковой строкой или подготовленным поисковым запросом, в соответствии с правилом результата [операторов сравнения](/ru/Comparison_operators_=_etc/.md) (`TRUE` или `NULL`, но никогда `FALSE`). Символы `%` и `_` в `compareExpr` не являются масками.
...
* `searchExpr`
  ...
  [Выражение](/ru/Expression/.md), значение которого определяет исходную строку или подготовленный поисковый вектор.
