...
[Направление поиска](/ru/Activation_ACTIVATE/.md#direction) задаётся одним из ключевых слов `FIRST`, `LAST` или `NULL` (значение `PREV` явно в операторе не указывается). Если ни одно из ключевых слов не задано, используется [тип объектов по умолчанию](/ru/Object_blocks/.md), заданный у группы объектов.
