...
Операторы изменений - набор операторов реализующих определение различных типов [изменений значений свойств](/ru/Change_operators_SET_CHANGED_etc/.md).
...
Операторы изменений создают [свойства](/ru/Properties/.md), которые определяют, произошли ли для некоторого свойства в текущей сессии те или иные виды изменений. Это [контекстно-зависимые](/ru/Property_operators/.md) операторы-свойства, которые могут использоваться внутри [выражений](/ru/Expression/.md). Внутри обработчика [события](/ru/Events/.md#change) они переключаются в событийный режим и сообщают об изменениях с момента предыдущего срабатывания этого события.
...
* `propExpr`
  ...
  [Выражение](/ru/Expression/.md), для которого проверяется наличие изменения. Оно должно задавать свойство; отдельный параметр не может быть операндом.
