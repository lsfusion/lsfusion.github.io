...
Оператор `READ` - создание [действия](/ru/Actions/.md), [считывающего файл](/ru/Read_file_READ/.md) из внешнего ресурса в [свойство](/ru/Properties/.md).
...
* `urlExpr`
  ...
  [Выражение](/ru/Expression/.md), значением которого является URL, из которого следует произвести чтение. Значение выражения должно быть строкового типа.

* `propertyId`

  [Идентификатор свойства](/ru/IDs/.md#propertyid), в которое будет производиться запись считанных данных. У этого свойства не должно быть параметров и класс его значения должен быть файловым (`FILE`, `RAWFILE`, `JSONFILE` и т.д.). Если свойство не указано, оно автоматически устанавливается равным `System.readFile`.
