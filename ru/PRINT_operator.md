...
Оператор `PRINT` - создание [действия](/ru/Actions/.md), [открывающего форму](/ru/In_a_print_view_PRINT/.md) в печатном представлении.
...
Оператор `PRINT` создает действие, которое печатает указанную форму. При печати формы в блоке `OBJECTS` можно добавлять объектам формы [дополнительные фильтры](/ru/Open_form/.md#params) на равенство этих объектов переданным значениям.
...
* `objName1 ... objNameN`
  ...
  Имена объектов формы, для которых задаются дополнительные фильтры. [Простые идентификаторы](/ru/IDs/.md#id).
...
#### Дополнительные опции[​](#дополнительные-опции "Прямая ссылка на этот заголовок")

* `printFormat`

  [Формат печати](/ru/In_a_print_view_PRINT/.md#format), задается одним из ключевых слов:
...
* `sheetProperty`
  ...
  [Идентификатор свойства](/ru/IDs/.md#propertyid), значение которого применяется в качестве названия листа в выгружаемом файле. У свойства не должно быть параметров. Используется для форматов печати `XLS`, `XLSX`.
...
* `propertyId`
  ...
  [Идентификатор свойства](/ru/IDs/.md#propertyid), в которое будет записан сформированный файл. У свойства не должно быть параметров. Если свойство не указано, сформированный файл передается клиенту и открывается у него средствами операционной системы

* `PREVIEW`

  Ключевое слово. Если указывается, то форма показывается в режиме [предварительного просмотра](/ru/In_a_print_view_PRINT/.md#interactive). Этот режим используется по умолчанию, если другие режимы / форматы не заданы.
...
* `MESSAGE`
  ...
  Ключевое слово. Если указывается, то форма выдает данные пользователю в режиме [сообщения](/ru/In_a_print_view_PRINT/.md#interactive).
