...
Платформа lsFusion может выступать в роли MCP-сервера ([Model Context Protocol](https://modelcontextprotocol.io/)) для внешних AI-клиентов (например, `claude.ai`). MCP-сервер работает поверх стандартного [HTTP API платформы](/ru/Access_from_an_external_system/.md), поэтому отдельный сервис разворачивать не требуется — достаточно сконфигурировать на сервере приложений два параметра: `enableAPI` и HTTPS.

### 1. Параметр `enableAPI`[​](#enableapi "Прямая ссылка на этот заголовок")

Параметр определяет доступ к программному интерфейсу, через который MCP-клиент обращается к серверу (`eval/action`, `exec` и т.п.). Полное описание — в разделе [Параметры работы](/ru/Working_parameters/.md).
...
#### Приоритетный способ: включение для конкретных ролей[​](#приоритетный-способ-включение-для-конкретных-ролей "Прямая ссылка на этот заголовок")
...
Задаётся это в работающей системе через форму `Администрирование > Система > Настройки > Параметры` — параметр `enableAPI` со значением `1` привязывается к нужной роли (механизм per-role настроек описан в [Параметрах работы](/ru/Working_parameters/.md)).

#### Альтернативные способы задания[​](#альтернативные-способы-задания "Прямая ссылка на этот заголовок")

Если по каким-то причинам per-role настройка не подходит, `enableAPI` можно выставить одним из стандартных способов (в порядке возрастания приоритета — см. [Параметры работы](/ru/Working_parameters/.md)):

* В `lsfusion.properties` проекта — `settings.enableAPI=1`.
* В `conf/settings.properties` конкретной инсталляции — `settings.enableAPI=1`.
* В [Java-параметрах запуска](/ru/Launch_parameters/.md#appjava) сервера приложений — `-Dsettings.enableAPI=1`.
...
### 2. HTTPS — обязателен[​](#https "Прямая ссылка на этот заголовок")
...
Развёртывание HTTPS выполняется по схеме, приведённой в конце [Автоматической установки](/ru/Execution_auto/.md). URL MCP-сервера, который прописывается в MCP-клиенте, должен начинаться на `https://` и резолвиться в публичный DNS-адрес с валидным сертификатом — самоподписанные сертификаты не подходят, клиент откажется устанавливать соединение.

### Пример URL[​](#example-url "Прямая ссылка на этот заголовок")
...
### Обнаружение авторизации при развёртывании не в корне[​](#well-known "Прямая ссылка на этот заголовок")
