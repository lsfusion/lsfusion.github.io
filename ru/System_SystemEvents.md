...
`SystemEvents` — [системный модуль](/ru/System_modules/.md), объявляющий обработчики событий запуска платформы, свойства версий платформы, настройки оформления по среде, а также системные журналы исключений, запусков сервера, подключений клиентов, сессий изменений и пингов компьютеров. Он же собирает действия push-уведомлений и взаимодействия с клиентом и пункты навигатора для логотипа, диалога оформления и форм журналов. Подключается через `REQUIRE SystemEvents` (`System`, `Reflection` и `Time` тянутся автоматически).

### События запуска[​](#события-запуска "Прямая ссылка на этот заголовок")

Эти абстрактные списки действий — обработчики [событий запуска](/ru/Launch_events/.md): разработчик встраивает в них логику инициализации, а платформа запускает нужный список в соответствующий момент запуска сервера или клиента. `onStarted[]` — обработчик запуска сервера приложений; полный набор обработчиков и моменты их срабатывания описаны в статье [`События запуска`](/ru/Launch_events/.md).
...
### Версии[​](#версии "Прямая ссылка на этот заголовок")
...
### Оформление[​](#оформление "Прямая ссылка на этот заголовок")

Настройки оформления разрешаются по `DesignEnv` — объекту среды оформления, представляющему выбранный одним клиентом внешний вид. Выбор задаётся тремя перечислимыми [классами](/ru/User_classes/.md):
...
### Исключения[​](#исключения "Прямая ссылка на этот заголовок")
...
### Запуски[​](#запуски "Прямая ссылка на этот заголовок")
...
### Подключения[​](#подключения "Прямая ссылка на этот заголовок")
...
### URL подключения[​](#url-подключения "Прямая ссылка на этот заголовок")
...
### Журнал сессий изменений[​](#журнал-сессий-изменений "Прямая ссылка на этот заголовок")
...
### Пинги[​](#пинги "Прямая ссылка на этот заголовок")
...
### Push-уведомления и взаимодействие с клиентом[​](#push-уведомления-и-взаимодействие-с-клиентом "Прямая ссылка на этот заголовок")
...
### Логотип[​](#логотип "Прямая ссылка на этот заголовок")
...
* [Заголовок модуля](/ru/Module_header/.md) — синтаксис `MODULE` / `REQUIRE`, подключающий модуль.
* [Оператор `ABSTRACT`](/ru/ABSTRACT_action_operator/.md) — абстрактные списки действий, которыми объявлены обработчики событий запуска.
* [Оператор `WHEN`](/ru/WHEN_statement/.md) — обработчики событий, реагирующие на изменения оформления и запускающие перезагрузку или изменение размера клиента.

### Связано[​](#связано "Прямая ссылка на этот заголовок")

* [`System modules`](/ru/System_modules/.md) — общий список модулей платформы.
* [`События запуска`](/ru/Launch_events/.md) — концепция событий запуска; объявленные этим модулем обработчики жизненного цикла (`onStarted`, `onWebClientStarted`, …) живут там.
* [`Журналы и логи`](/ru/Journals_and_logs/.md) — системные журналы, которые наполняет этот модуль (исключения, запуски, подключения, сессии, пинги).
* [`Монитор процессов`](/ru/Process_monitor/.md) — мониторинг работающих подключений и активности сервера.
* [`Навигатор`](/ru/Navigator/.md) — пункты навигатора, которые добавляет этот модуль (логотип, оформление, журналы).
* [`Service`](/ru/System_Service/.md) — служебные действия и мониторинг базы данных.
* [`Reflection`](/ru/System_Reflection/.md) — метаданные о навигаторе, формах и свойствах.
* [`Authentication`](/ru/System_Authentication/.md) — пользователи, контакты и вход.
