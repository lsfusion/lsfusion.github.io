...
`Time` — [системный модуль](/ru/System_modules/.md), собирающий свойства, действия и классы для работы со временем: получение текущей даты-времени, преобразования между классами времени, извлечение компонентов даты, арифметика над датами и временем, интервалы, календарные классы (месяцы, дни недели). Подключается через `REQUIRE Time` (`System` тянется автоматически).
...
### Извлечение компонентов[​](#извлечение-компонентов "Прямая ссылка на этот заголовок")
...
### Арифметика[​](#арифметика "Прямая ссылка на этот заголовок")
...
### Итерация и интервалы[​](#итерация-и-интервалы "Прямая ссылка на этот заголовок")
...
### Календарные классы и формы[​](#календарные-классы-и-формы "Прямая ссылка на этот заголовок")

`Month` и `DOW` — встроенные [классы](/ru/User_classes/.md#static), описывающие месяцы (12 объектов) и дни недели (7 объектов). Содержат свойства `number[Month]`, `name[Month]`, `number[DOW]`, `numberM[DOW]` (нумерация с понедельника), `name[DOW]`. Поиск по номеру: `month[INTEGER]`, `DOW[INTEGER]`. Формы `months` и `DOWs` показывают эти классы как справочники.

`DateTimePickerRanges` и `DateTimeIntervalPickerRanges` — статические [классы](/ru/User_classes/.md#static) с готовыми наборами шорткатов (`rangeToday`, `rangeYesterday`, `rangeLast7Days` и т. п.) для выбора в UI-пикерах одиночной даты и интервала.

### Связано[​](#связано "Прямая ссылка на этот заголовок")

* [`System modules`](/ru/System_modules/.md) — общий список модулей платформы.
* [`Built-in classes`](/ru/Built-in_classes/.md) — описание классов `DATE`, `TIME`, `DATETIME`, `ZDATETIME`, `INTERVAL[…]`.
