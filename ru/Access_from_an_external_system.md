...
Расширение файла при этом определяется автоматически по аналогии с [оператором `WRITE`](/ru/WRITE_operator/.md).
...
##### Аутентификация[​](#аутентификация "Прямая ссылка на этот заголовок")
...
Возможность выполнения запроса также зависит от настройки [`enableAPI`](/ru/Working_parameters/.md), которая позволяет по умолчанию отключить API, ограничить его авторизованными запросами или дополнительно разрешить анонимные запросы. Отдельное действие можно пометить [`@@noauth`](/ru/Action_options/.md), чтобы обойти и проверку аутентификации, и `enableAPI`, или [`@@api`](/ru/Action_options/.md), чтобы разрешить его при `enableAPI=0` (всё ещё требуя авторизованного пользователя). При `enableAPI=0` внешний API также разрешается авторизованному пользователю, у которого есть доступ к форме навигатора `System.interpreter`, поскольку такой пользователь уже может выполнять произвольный lsFusion-код через браузерный UI.
...
* [`@@ui`](/ru/Action_options/.md) - всегда интерактивно.
* [`@@noui`](/ru/Action_options/.md) - никогда не определяется как интерактивное, и действие выполняется синхронно (если только клиент явно не запросил идентификатор уведомления через заголовок `Need-Notification-Id` - он обрабатывается всегда).
