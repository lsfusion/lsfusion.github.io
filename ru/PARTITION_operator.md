...
Оператор `PARTITION` - создание [свойства](/ru/Properties/.md), реализующего [разбиение / упорядочивание](/ru/Partitioning_sorting_PARTITION_..._ORDER/.md) либо [простое распределение](/ru/Distribution_UNGROUP/.md).
...
Оператор `PARTITION` создаёт свойство, которое для каждого набора объектов либо вычисляет [агрегирующую функцию](/ru/Set_operations/.md#func) по окну разбиения (`SUM`, `PREV`, `LAST`, `CUSTOM`), либо распределяет некоторое значение среди наборов объектов группы (`UNGROUP`).
...
Блок `ORDER` определяет порядок, в котором будет либо вычисляться агрегирующая функция, либо проходить распределение. Если эта функция [некоммутативна](/ru/Set_operations/.md), заданный порядок должен быть однозначно определяемым. Если в выражениях, задающих порядок, объявляется новый параметр (не встречавшийся ранее в опциях `PARTITION` и `BY` и в верхнем контексте), то при вычислении результирующего значения автоматически добавляется условие на не-`NULL` всех этих выражений.
...
* `propertyId`
  ...
  [Идентификатор](/ru/IDs/.md#propertyid) распределяемого свойства. Значение этого свойства должно быть числового типа, а количество параметров должно совпадать с количеством группировок в блоке `BY`. На вход этому свойству передаются объекты, идентифицирующие группу.
...
* `digits`
  ...
  [Целочисленный литерал](/ru/Literals/.md#intliteral), указывающий количество знаков после запятой при округлении в `PROPORTION`.
