...
**Явно — перечислите его в *onWebClientInit*.** Любой другой файл в *src/main/resources/web* — вне *web/init* или внутри исключённой *web/init/lib* — загружается указанием его имени в действии [`onWebClientInit`](/ru/INTERNAL_operator/.md) с целочисленным порядком. Используйте это, когда важен порядок загрузки — сторонняя библиотека должна загрузиться раньше использующего её компонента — или для условной загрузки файла:
