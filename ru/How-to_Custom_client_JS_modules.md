...
В отличие от обычной сборки — которая использует нативный бинарник esbuild и не требует Node — этот проход выполняется через Node, который сборка **получает автоматически**: использует Node из `PATH`, если он есть, иначе один раз скачивает закреплённый Node с проверкой контрольной суммы и кэширует его (в *\~/.m2*, так что последующие сборки и кэш *\~/.m2* на CI его переиспользуют). Поэтому достаточно включить флаг — устанавливать Node вручную на машинах разработчиков или CI не нужно. Исключение — офлайн-сборка (`mvn -o`): она ничего не качает, поэтому если Node при этом нет ни в `PATH`, ни в кэше, сборка падает с подсказкой (установить Node или один раз собрать онлайн, чтобы заполнить кэш). Node нужен только во время сборки — машине времени выполнения он не нужен. (При необходимости у плагина можно переопределить `nodeVersion` и `nodeDownloadRoot` — зеркало.)

Проход **выключен по умолчанию, и большинству приложений он не нужен**: типичный кейс производительности больших таблиц — перерисовка только реально изменившихся строк — уже решается [`window.lsfusion.List`](/ru/How-to_Custom_React_views/.md) независимо от этого прохода, и `List` остаётся правильным инструментом для этого независимо от того, включён компилятор или нет. Включайте `reactCompiler`, когда в самом пользовательском представлении React достаточно вычисляемых значений, колбэков или вложенных поддеревьев, чтобы выиграть от общей автомемоизации.
