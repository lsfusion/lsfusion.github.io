...
* `CUSTOM customExpr`
  ...
  * `customExpr`
    ...
    Шаблон здесь тот же, который принимает атрибут `custom` контейнера [`DESIGN`](/ru/DESIGN_statement/.md), с теми же правилами записи места; здесь `<Lsf:name>` — место, куда попадает стандартная кнопка элемента навигатора с этим именем. В литерале имя разрешается в [пространствах имён](/ru/Naming/.md#namespace) модуля, поэтому элемент должен быть уже объявлен, а имя, которое ни во что не разрешается, — ошибка; шаблон, вычисляемый свойством, не разрешается, и там пишется [каноническое имя](/ru/Naming/.md#canonicalname).
