...
### Нумерация вручную[​](#нумерация-вручную "Прямая ссылка на этот заголовок")
...
[Оператор `GROUP AGGR`](/ru/Grouping_GROUP/.md) автоматически добавляет ограничение на уникальность номера. При попытки записать в базу данных повторный номер будет выдано сообщение об ошибке.

Добавляем [событие](/ru/Events/.md), которое будет автоматически проставлять книге номер, равный следующий за максимальным из существующих в базе данных.
...
Готовая реализация этого подхода — серия, ведущие нули, нумераторы по умолчанию и защищённая от конфликтов генерация — поставляется с платформой в модуле [`Numerator`](/ru/Utils_Numerator/.md). Приведённые ниже сценарии опираются на него; в каждом из них номер проставляется, только если он ещё не задан, поэтому введённое пользователем или загруженное импортом значение никогда не перезаписывается.

### Порядковый номер для класса[​](#порядковый-номер-для-класса "Прямая ссылка на этот заголовок")
...
### Отдельная последовательность для каждого типа документа[​](#отдельная-последовательность-для-каждого-типа-документа "Прямая ссылка на этот заголовок")
...
### Сгенерированный код как идентификатор объекта[​](#сгенерированный-код-как-идентификатор-объекта "Прямая ссылка на этот заголовок")
