...
Далее используются свойства-обёртки над PostgreSQL-функциями `jsonb_*`, которые платформа поставляет в модуле `Utils`: `field`, `fieldText`, `array`, `arrayText`, `map`, `mapText`, `arrayElement`. Их сигнатуры и описание собраны в [`Utils` → `Свойства доступа к JSON`](/ru/System_Utils.md#json-access); этот how-to опирается на них и не повторяет их интерфейс.
...
JSON той же структуры, что и в [примере 1](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-1). Нужно построчно обойти все заказы и в каждом — все его строки.
...
В отличие от формового варианта, staging-свойства здесь не нужны; и [`imported[INTEGER]`](/ru/IMPORT_operator.md) тоже отсутствует, потому что итерации в явном виде нет — её роль выполняет сама `DO`-часть.

`FIELDS … DO` стоит выбирать, когда значения нужны ровно один раз и валидация не требует нескольких проходов. Если потребуется сначала проверить ссылки, потом пакетно создать объекты, и наконец заполнить их свойства — переключиться на форму или на промежуточные `LOCAL` (см. [пример 5](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-5); для массива на корневом уровне, как здесь, — [пример 6](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-6)).
...
Этот приём заменяет полноценный формовый импорт, пока вложенность ограничена объектами в строках массива. Как только в JSON появляются вложенные массивы, по которым тоже нужно итерироваться, — без формы или внешнего `FOR JSON … = array(...)` (см. [пример 2](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-2)) обойтись уже сложнее.
...
Аналогично [**примеру 5**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-5), но файл представляет собой JSON-массив на корневом уровне — в таком виде списки обычно возвращают внешние сервисы:
...
Форма отличается от формы из [примера 5](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-5) только опцией `EXTID 'value'` у группы объектов. Группа объектов читает свои записи из массива под ключом, равным её [имени экспорта / импорта](/ru/Structured_view.md#extid), а у массива на корневом уровне ключа нет: такой файл платформа читает как `{ "value" : [ ... ] }` (см. [преобразование предопределённого значения `value`](/ru/Structured_view.md#value)), поэтому читающая его группа должна иметь имя экспорта / импорта `value`. Собственное имя объекта `books` при этом сохраняется: `EXTID` меняет только имя, под которым группа ищется в файле.

С любым другим именем импорт завершается без ошибки и не читает ни одной строки: группа объектов, имени которой не соответствует ни один ключ, пропускается вместе со всем, что под ней. То же происходит в [примере 5](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-5), если группа названа не так, как ключ, и тогда, когда `ROOT` указывает на ключ самого массива, — `IMPORT importBooks JSON ROOT 'books' FROM f` делает корнем этот массив, и он снова читается только группой с именем `value`. О таком несовпадении ничто не сообщает, поэтому действие, прежде чем продолжить, проверяет, что прочитана хотя бы одна строка. Отличить несовпавшее имя от файла с пустым массивом эта проверка не может, поэтому она уместна там, где пустой файл не ожидается.

Варианту с `FIELDS` из [примера 3](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-3) такое имя не нужно: группа объектов формы, которую он создаёт автоматически, уже называется `value`.

## Пример 7[​](#пример-7 "Прямая ссылка на этот заголовок")

### Задача[​](#задача "Прямая ссылка на этот заголовок")
...
### Решение[​](#решение-6 "Прямая ссылка на этот заголовок")
...
Каждый вложенный массив получает собственный блок `OBJECTS` по `INTEGER` — ровно как единственный массив в [примере 5](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-5). Связью с родительским уровнем служит `LOCAL`-свойство от дочерней строки к родительской (`eventDocflow`, `participantEvent`), указанное в `FILTERS`: при импорте платформа заполняет его строкой охватывающего элемента массива. Этот же фильтр делает дочернюю группу потомком родительской при построении [иерархии групп объектов](/ru/Static_view.md#hierarchy) — именно он направляет чтение массива `events` из ключа `events` внутри каждого элемента `docflows`, а не из корня формы.

Самый внутренний массив состоит из строк, а не объектов; по [преобразованию предопределённого значения `value`](/ru/Structured_view.md#value) каждый такой элемент читается как объект `{ "value" : ... }`, поэтому промежуточное свойство отображается через `EXTID 'value'`.

Если вложенный массив лежит не прямо в элементе, а под промежуточным ключом-объектом (скажем, `"status": {"details": [...]}`), объявите [группу свойств](/ru/GROUP_statement.md) с этим именем экспорта / импорта и добавьте дочерний блок `OBJECTS` в неё через `IN`. Группа вкладывается под группу объектов итерируемого элемента — по тому же построению иерархии, — а не под корень формы.

## Пример 8[​](#пример-8 "Прямая ссылка на этот заголовок")

### Задача[​](#задача-1 "Прямая ссылка на этот заголовок")

Аналогично [**Примеру 7**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-7), но ни одно поле записи не обязательно: документооборот может прийти без `number`, а событие — пустым объектом.
...
### Решение[​](#решение-7 "Прямая ссылка на этот заголовок")
