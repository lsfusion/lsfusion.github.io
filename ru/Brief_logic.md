...
[Оператор `RECURSION`](/ru/RECURSION_operator.md) создает свойство, вычисляемое итерациями; к нему обращаются на деревьях, графах и транзитивных замыканиях — все предки объекта, достижимость по цепочке ссылок, а через подсчёт предков — уровень узла. Его части — `STEP`, префикс `$` перед параметром и опция `CYCLES` — и то, как считаются итерации, описаны в [рекурсии](/ru/Recursion_RECURSION.md). Для иерархии по свойству `parent[class]` готовый набор таких свойств даёт системный модуль [`Hierarchy`](/ru/Utils_Hierarchy.md).
...
### Периодическое выполнение[​](#periodic-execution "Прямая ссылка на этот заголовок")
...
| ------------------------------------------------------------------------------------ | ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `EVENTS ON SCHEDULE PERIOD n` [формы](/ru/Form_events.md)                            | на клиенте, у каждой открытой формы                           | на сервере, в сессии формы; асинхронный запрос, не блокирующий пользователя                                                              | к форме: выполняется, только пока она показана на экране, скрытая форма пропускает запуски |
| `SCHEDULE PERIOD n` в [`NAVIGATOR`](/ru/NAVIGATOR_statement.md)                      | на клиенте, у соединения                                      | на сервере, каждый раз в новой сессии; синхронный запрос: ввод заблокирован на время выполнения, индикатор ожидания — после его задержки | к соединению: выполняется, пока пользователь в системе, какие бы формы ни были открыты     |
| `NEWTHREAD ... SCHEDULE PERIOD` ([потоки](/ru/New_threads_NEWTHREAD_NEWEXECUTOR.md)) | в серверном пуле либо на клиенте при `NEWEXECUTOR ... CLIENT` | на сервере, в сессии вызывающего кода (пул) либо в новой сессии соединения (`CLIENT`)                                                    | к объемлющему `NEWEXECUTOR` (при `CLIENT` — к соединению)                                  |
| [планировщик](/ru/Scheduler.md)                                                      | на сервере                                                    | на сервере, без участия клиента                                                                                                          | к серверу: задача, настроенная администратором                                             |
