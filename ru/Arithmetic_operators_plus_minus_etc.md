...
*Арифметические операторы* создают [свойства](/ru/Properties.md), значением которых является результат арифметической операции. В основном они работают со значениями [числовых классов](/ru/Built-in_classes.md). Кроме того, сумма и разность работают также со значениями даты/времени (описано ниже), а сумма ещё и конкатенирует строки (см. [Строковые операторы](/ru/String_operators_plus_CONCAT_SUBSTRING.md)). В платформе на данный момент поддерживаются следующие арифметические операторы:
...
Остаток от деления, целочисленное деление и возведение в степень отдельными операторами не представлены — их выполняют свойства системного модуля [`Utils`](/ru/System_Utils.md):
...
| Оператор | Результат                                                                                             |
| -------- | ----------------------------------------------------------------------------------------------------- |
| `+`, `-` | [Общий предок](/ru/Built-in_classes.md#commonparentclass) (семейство - Числа)                         |
...
Если хотя бы один операнд оператора `+` принадлежит строковому классу, результатом является строковый класс; его длина, регистронезависимость и остальные параметры определяются по правилам объединения строк — см. [строковые операторы](/ru/String_operators_plus_CONCAT_SUBSTRING.md).

### Язык[​](#язык "Прямая ссылка на этот заголовок")

Описание [арифметических операторов](/ru/Arithmetic_operators.md).
