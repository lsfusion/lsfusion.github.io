...
Строка, получаемая из `expression`, оборачивается платформой в модуль с уникальным именем, зависимости которого охватывают все загруженные модули проекта, и затем выполняется. Без `ACTION` код трактуется как последовательность [инструкций](/ru/Statements/.md); одна из них должна объявлять действие с именем `run` — именно оно и будет выполнено. С `ACTION` код трактуется как тело самого действия `run` — последовательность [операторов-действий](/ru/Action_operators/.md) и объявлений локальных свойств, — а runtime-аргументы адресуются позиционно как `$1, $2, ...`.
...
* `expression`
  ...
  [Выражение](/ru/Expression/.md), значением которого является строка с исходным кодом, подлежащим выполнению.
