...
17. Класс результата выражения может оказаться шире классов, из которых оно построено, и ассистент ОБЯЗАН учитывать это везде, где требуется более узкий класс, — прежде всего в реализации `+=`, где это ошибка старта сервера.
    ...
    * `/` расширяет катастрофически: при настройках по умолчанию его шкала всегда равна максимальной шкале `NUMERIC` (`32`), поэтому `NUMERIC[16,2] / NUMERIC[16,2]` — это `NUMERIC[48,32]`.
    ...
    Более узкий класс получается только явным приведением всего выражения. Для операндов целочисленных классов приведение операнда из правила 16 не ограничивает результат (деление всё равно расширяется до шкалы `32`), поэтому нужны оба приведения: `NUMERIC[16,2](NUMERIC[16,2](a(x)) / b(x))`.

18. Класс в объявлении параметра (`prop(SubClass x)`) — это сигнатура, а не фильтр времени выполнения: он разрешает одноимённые свойства и задаёт сигнатуру, но вычисляемое множество определяется свойствами, использованными в выражении. Чтение свойства родительского класса с параметром, объявленным классом-потомком, всё равно идёт по ВСЕМ объектам родительского класса (например, в `GROUP SUM` — молча неверные итоги).
...
19. В операторе `GROUP ... BY` ассистент НЕ ДОЛЖЕН перечислять в блоке `BY` верхние параметры, использованные в выражениях оператора: каждый такой параметр уже неявно является группировкой — параметром создаваемого свойства — и остаётся на своём месте в сигнатуре.
...
20. `MAX` и `MIN` — префиксные операторы над списком операндов через запятую (`MAX a, b`), а не инфиксные: `a MAX b` не разбирается — платформа выдаёт `no viable alternative at input 'MAX'`.
...
21. `AND`, `OR`, `XOR` и `NOT` всегда дают `BOOLEAN` (`TRUE` или `NULL`), а не значение операнда: `name(o) AND active(o)` равно `TRUE`, а не имени, `a OR b` равно `TRUE`, а не первому непустому значению. Ассистент НЕ ДОЛЖЕН использовать их для выбора или пропуска значения; для этого служат `expr IF cond` и `OVERRIDE a, b`.
...
1. Класс значения реализации `+=` ОБЯЗАН укладываться в класс значения, объявленный у абстрактного свойства; неявного приведения нет — реализация с более широким классом отвергается при старте сервера с ошибкой «wrong value class of implementation», в строках `specified` и `expected` которой указаны класс реализации и объявленный класс.
   ...
   Выражение, расширяющее класс значения, — прежде всего арифметику, и в первую очередь деление (правило 17 правил свойств), — ассистент ОБЯЗАН обернуть в явное приведение к объявленному классу: `f(X x) += NUMERIC[16,2](a(x) / b(x));` `f(X x) += ISTRING[250](a(x) + b(x));`
