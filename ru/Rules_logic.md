...
* [Правила: свойства](#%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D1%81%D0%B2%D0%BE%D0%B9%D1%81%D1%82%D0%B2%D0%B0)
* [Правила: действия и присваивание](#%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D0%B4%D0%B5%D0%B9%D1%81%D1%82%D0%B2%D0%B8%D1%8F-%D0%B8-%D0%BF%D1%80%D0%B8%D1%81%D0%B2%D0%B0%D0%B8%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5)
* [Правила: события (WHEN)](#%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D1%81%D0%BE%D0%B1%D1%8B%D1%82%D0%B8%D1%8F-when)
* [Правила: ограничения](#%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D0%BE%D0%B3%D1%80%D0%B0%D0%BD%D0%B8%D1%87%D0%B5%D0%BD%D0%B8%D1%8F)
* [Правила: сессии изменений (NEWSESSION, APPLY)](#%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D1%81%D0%B5%D1%81%D1%81%D0%B8%D0%B8-%D0%B8%D0%B7%D0%BC%D0%B5%D0%BD%D0%B5%D0%BD%D0%B8%D0%B9-newsession-apply)

## Правила: свойства[​](#правила-свойства "Прямая ссылка на этот заголовок")

### Правила свойств[​](#правила-свойств "Прямая ссылка на этот заголовок")
...
8. Чтобы проверить, что свойство равно `NULL`, ассистенту СЛЕДУЕТ использовать `IF NOT property(...)`.

   Чтобы проверить, что оно не `NULL`, ассистенту СЛЕДУЕТ использовать `IF property(...)`.

   Это относится к свойству любого класса, а не только `BOOLEAN`: условие выполнено, когда его значение не `NULL`, каким бы это значение ни было — `hasNotes(o) = IF notes(o) THEN TRUE` для `STRING`, `isScheduled(o) = IF scheduledAt(o) THEN TRUE` для `DATETIME`, `hasDepartment(e) = IF department(e) THEN TRUE` для свойства со значением-объектом, `NOT query() OR name(o) LIKE ('%' + query() + '%')` для поискового фильтра, пропускающего всё, пока запрос пуст (`NOT` и `OR` не распространяют `NULL`, правило 3).

   Ассистент НЕ ДОЛЖЕН сравнивать с литералом `NULL`: `expr = NULL`, `expr == NULL` и `expr != NULL` — обычные сравнения, и по правилу 3 операнд `NULL` делает результат `NULL`, что бы ни содержал `expr` — `scheduledAt(o) != NULL` никогда не `TRUE`, поэтому `FILTER` или `WHERE` на нём ничего не отбирают, `SHOWIF` на нём всегда скрывает, `IF` на нём всегда уходит в ветку `ELSE` — а сервер лишь выдаёт предупреждение при загрузке модуля, код по-прежнему компилируется и запускается.

9. Ассистенту СЛЕДУЕТ указывать `CHARWIDTH` в определении свойства, а не в дизайне формы.
...
10. Для статических объектов ассистент НЕ ДОЛЖЕН использовать свойства `staticCaption` или `staticName`.
...
11. Ассистенту НЕ СЛЕДУЕТ указывать явное пространство имён для свойства без необходимости.

12. Создавая для собственного атрибута одного объекта DATA-свойство — или простую композицию от DATA-свойства (например, когда тянется имя связанного объекта), — ассистент ОБЯЗАН осознанно решить, относить ли его к системной группе `id` или `base` через `IN`.
...
13. При делении значений целочисленных классов ассистент ОБЯЗАН приводить к `NUMERIC` один из операндов, а не результат.
...
14. Класс результата выражения может оказаться шире классов, из которых оно построено, и ассистент ОБЯЗАН учитывать это везде, где требуется более узкий класс (прежде всего в реализации `+=`: правила абстрактных свойств).
    ...
    * `/` расширяет катастрофически: при настройках по умолчанию его шкала всегда равна максимальной шкале `NUMERIC` (`32`), поэтому `NUMERIC[16,2] / NUMERIC[16,2]` — это `NUMERIC[48,32]`.
    ...
    Агрегат `GROUP` в основном сохраняет класс того, что агрегирует, — `GROUP SUM`, `GROUP MAX` или `GROUP LAST` по `NUMERIC[16,2]` дают `NUMERIC[16,2]`, — но выносит наружу то, до чего расширилось агрегируемое выражение. Сам по себе расширяет `GROUP CONCAT`: его результат — строка неограниченной длины. Конкатенация строк тоже расширяет, складывая длины операндов: `ISTRING[200] + ISTRING[126]` — это `ISTRING[326]`.

    Более узкий класс получается только явным приведением всего выражения. Для операндов целочисленных классов приведение операнда из правила 13 не ограничивает результат (деление всё равно расширяется до шкалы `32`), поэтому нужны оба приведения: `NUMERIC[16,2](NUMERIC[16,2](a(x)) / b(x))`.

15. Класс в объявлении параметра (`prop(SubClass x)`) — это сигнатура, а не фильтр времени выполнения: он разрешает одноимённые свойства и задаёт сигнатуру, но вычисляемое множество определяется свойствами, использованными в выражении. Чтение свойства родительского класса с параметром, объявленным классом-потомком, всё равно идёт по ВСЕМ объектам родительского класса (например, в `GROUP SUM` — молча неверные итоги).
...
16. В операторе `GROUP ... BY` ассистент НЕ ДОЛЖЕН перечислять в блоке `BY` верхние параметры, использованные в выражениях оператора: каждый такой параметр уже неявно является группировкой — параметром создаваемого свойства — и остаётся на своём месте в сигнатуре.
    ...
    Во встроенной форме `[GROUP ... BY ...](...)` такие параметры передаются автоматически: аргументы соответствуют по порядку только `BY`-выражениям (`[GROUP SUM f(x) IF g(x) = s BY h(x)](y)`), а указание `s` среди них даёт ошибку количества параметров. Ассистент ОБЯЗАН убедиться, что имя, использованное в скобках без класса, уже объявлено снаружи, раньше по тексту: иначе оно молча становится параметром самого `GROUP`, и агрегат считается по всем его значениям.

17. `MAX` и `MIN` — префиксные операторы над списком операндов через запятую (`MAX a, b`), а не инфиксные: `a MAX b` не разбирается — платформа выдаёт `no viable alternative at input 'MAX'`.
...
18. `AND`, `OR`, `XOR` и `NOT` всегда дают `BOOLEAN` (`TRUE` или `NULL`), а не значение операнда: `name(o) AND active(o)` равно `TRUE`, а не имени, `a OR b` равно `TRUE`, а не первому непустому значению. Ассистент НЕ ДОЛЖЕН использовать их для выбора или пропуска значения; для этого служат `expr IF cond` и `OVERRIDE a, b`.

### Правила абстрактных свойств (`+=`)[​](#правила-абстрактных-свойств- "Прямая ссылка на этот заголовок")
...
1. Класс значения реализации `+=` ОБЯЗАН укладываться в класс значения, объявленный у абстрактного свойства; неявного приведения нет — реализация с более широким классом отвергается при старте сервера с ошибкой «wrong value class of implementation», в строках `specified` и `expected` которой указаны класс реализации и объявленный класс.
   ...
   Выражение, расширяющее класс значения, — прежде всего арифметику, и в первую очередь деление (правило 14 правил свойств), — ассистент ОБЯЗАН обернуть в явное приведение к объявленному классу: `f(X x) += NUMERIC[16,2](a(x) / b(x));` `f(X x) += ISTRING[250](a(x) + b(x));`

### Правила упорядочивания (`ORDER`)[​](#правила-упорядочивания-order "Прямая ссылка на этот заголовок")
...
4. `PARTITION` не разделяет окно по параметрам свойства: чтобы нумеровать строки отдельно для каждого `loc` в `idx(loc, x) <- PARTITION SUM 1 IF cond(loc, x) ORDER x`, ассистент ДОЛЖЕН добавить `BY loc`; без него нумерация идёт сквозь все значения `loc`. Строки, у которых суммируемое выражение `NULL`, в окно не входят, поэтому `SUM 1 IF cond` сам по себе нумерует, при уникальном порядке, строки, где выполняется `cond`, с 1.

## Правила: действия и присваивание[​](#правила-действия-и-присваивание "Прямая ссылка на этот заголовок")

### Правила действий[​](#правила-действий "Прямая ссылка на этот заголовок")
...
2. Параметры, вводимые в `NEW alias = Class` и `FOR expr(p) [NEW alias = Class] DO { ... }`, НЕ следуют обычным правилам лексической области видимости из мейнстримных языков программирования.
   ...
   И наоборот, параметр, объявленный внутри агрегата `GROUP`, принадлежит этому агрегату и НЕ виден снаружи; в частности, он не может служить переменной цикла объемлющего `FOR`. Объявите переменную собственным параметром `FOR`, а агрегат используйте только как логическое условие по ней. Чтобы перебрать группы агрегата вместе с его значением, встроенную форму применяют к новым типизированным параметрам: `FOR NUMERIC[16,2] q = [GROUP SUM f(x) BY h(x)](Class y) DO ...`.
...
7. Параметр, введённый локально верхнеуровневым оператором тела действия (неявным циклом присваивания, `FOR`, `NEW`), виден только внутри этого оператора: то же имя в следующем операторе — новый параметр со своим классом.

   В генерируемых скриптах (`eval`, наполнение данных) ассистенту всё равно СЛЕДУЕТ давать таким параметрам уникальные имена, чтобы класс каждого был очевиден в месте использования.
...
### Правила присваивания (`<-`)[​](#правила-присваивания-- "Прямая ссылка на этот заголовок")
...
1. Аргументы изменяемого свойства в левой части `<-` могут быть выражениями от параметров оператора (`sentFolder(account(f)) <- f`), но новые локальные параметры вводятся только как типизированные параметры, а не внутри выражений. Запись «по вычисляемому ключу» по аналогии с императивным `map[key] = value` легко нарушает это.
   ...
   Ключ-выражение от нового параметра записывают из цикла, который вводит этот параметр: `FOR cond(Cls x) DO out(rowNum(x)) <- value(x);` (`rowNum[Cls]` посчитан заранее, например `PARTITION SUM 1`).
...
### Правила циклов (`FOR`, `WHILE`)[​](#правила-циклов-for-while "Прямая ссылка на этот заголовок")
...
### Правила потоков (`NEWTHREAD`, `NEWEXECUTOR`)[​](#правила-потоков-newthread-newexecutor "Прямая ссылка на этот заголовок")
...
## Правила: события (WHEN)[​](#правила-события-when "Прямая ссылка на этот заголовок")

### Правила событий (`WHEN`)[​](#правила-событий-when "Прямая ссылка на этот заголовок")
...
### Когда на самом деле выполняются локальные события[​](#когда-на-самом-деле-выполняются-локальные-события "Прямая ссылка на этот заголовок")
...
## Правила: ограничения[​](#правила-ограничения "Прямая ссылка на этот заголовок")
...
## Правила: сессии изменений (NEWSESSION, APPLY)[​](#правила-сессии-изменений-newsession-apply "Прямая ссылка на этот заголовок")

1. Прежде чем вводить `NEWSESSION`, ассистент ОБЯЗАН решить, какое поведение сессии требуется. Ни один из вариантов ниже — включая `NEWSQL` — не работает внутри транзакции `APPLY`: в обработчике глобального события или в применяемом действии сессия не создается вовсе, внутреннее действие откладывается и выполняется в текущей сессии, в той же транзакции. Ассистент НЕ ДОЛЖЕН рассчитывать там на независимую фиксацию.
   ...
   * дочерний диалог или редактор, который должен работать с несохранёнными объектами верхней сессии и возвращать свои изменения в эту верхнюю сессию -> `NESTEDSESSION`; ассистент НЕ ДОЛЖЕН заменять его простым `NEWSESSION`, пока родительский объект может быть ещё не сохранён в сессии формы

2. Простой `NEWSESSION` — вариант по умолчанию для изолированной работы, которая не должна случайно применить ожидающие изменения формы вызывающей стороны:
   ...
   * небольшие немедленные обновления UI с `NEWSESSION { APPLY { ... } }`
   ...
   Действие, запущенное на форме с редактируемыми свойствами, ДОЛЖНО либо быть полностью независимым от несохранённых изменений этой формы, либо сначала сохранить их: `APPLY;` `IF canceled() THEN RETURN;` `NEWSESSION { ... }` Это паттерн перед сменой статусов, созданием зависимых документов и другими изолированными последующими действиями.

3. Если внутренняя логика зависит от локального состояния верхней сессии, такого как выделения, отметки или буферы импорта, ассистент ОБЯЗАН явно перенести это состояние через `NESTED (...)` или `NESTED LOCAL` в операторе либо объявить само свойство `DATA LOCAL NESTED` — такое переносится без перечисления в операторе. Под `NEWSQL` не работает ни то, ни другое: на собственном соединении не переносится ничего, поэтому ассистент НЕ ДОЛЖЕН сочетать `NEWSQL` с зависимостью от локального состояния верхней сессии.

4. Успешный `APPLY` очищает сессию, а с ней и все обычные `LOCAL`-свойства в ней: после возврата из такого `APPLY` `LOCAL` снова пуст. Значение `LOCAL` переживает успешный `APPLY` только когда он объявлен как `NESTED` (`LOCAL NESTED name = Type ();` или `name = DATA LOCAL NESTED Type (...);`) либо когда `APPLY` явно сохраняет его — `APPLY NESTED (name1, ..., nameN)` или `APPLY NESTED LOCAL` для всех локальных. Накопленное значение, которое должно жить дольше `APPLY`, — например, буфер импорта, читаемый в пост-apply доработке, — ОБЯЗАНО идти одним из этих путей; это же касается локальных, перенесённых через `NEWSESSION NESTED (...)` или `NEWSESSION NESTED LOCAL`, когда их результат должен быть скопирован обратно в верхнюю сессию: обратно копируются уже очищенные значения.

   `APPLY`, который не прошел или был отменен, оставляет сессию как была, вместе с локальными, — поэтому ассистент НЕ ДОЛЖЕН читать `LOCAL` после `APPLY`, чтобы отличить успех от неудачи; их различает `canceled()`. Внутри вложенной сессии очистки нет вовсе: изменения копируются в родительскую сессию, а вложенная остается стоять вместе со своими локальными.

5. После `APPLY` ассистент ОБЯЗАН проверять `canceled()` только тогда, когда последующая логика зависит от того, удалось ли сохранение — для раннего выхода, пропуска побочного эффекта или отката отложенной работы.
...
6. Ассистенту СЛЕДУЕТ держать блоки `NEWSESSION` небольшими и целевыми: изолировать одну единицу работы, применить её при необходимости и выйти.
...
7. Тело `APPLY` может выполниться не один раз. Транзакция применения МОЖЕТ быть автоматически повторена после update conflict, deadlock или таймаута — будет ли, зависит от сбоя и от лимита попыток, — а применяемое действие и синхронные глобальные обработчики находятся внутри того, что повтор выполняет заново.
