...
19. `MAX` и `MIN` — префиксные операторы над списком операндов через запятую (`MAX a, b`), а не инфиксные: `a MAX b` не разбирается — платформа выдаёт `no viable alternative at input 'MAX'`.
