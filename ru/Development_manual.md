...
## Установка в существующих Java-проектах[​](#установка-в-существующих-java-проектах "Прямая ссылка на этот заголовок")

### Установка сервера приложений через IDE[​](#existingide "Прямая ссылка на этот заголовок")
...
* Если сервер БД находится на другом компьютере, а также если на сервере БД включена авторизация (например, для Postgres, по методу md5 и пароль postgres не пустой), задать [параметры подключения к серверу БД](/ru/Launch_parameters/.md#connectdb) (например, создав [файл настроек](/ru/Launch_parameters/.md#filesettings) запуска в папке проекта)
...
Если работа идет с большим проектом и в нем активно используются [метакоды](/ru/Metaprogramming/.md) (например [ERP](https://github.com/lsfusion-solutions/erp)), рекомендуется в параметрах запуска IDEA (`idea.exe.vmoptions` или `idea64.exe.vmoptions`) увеличить `Xmx` хотя бы до 2gb.
