# Действия

*Действие* - это элемент системы, который принимает на вход набор объектов (*параметров*) и с их использованием тем или иным способом изменяет состояние системы (как той в которой действие выполняется, так и состояние любой другой внешней системы).

Тип и поведение каждого действия определяется [оператором](/ru/Action_operators_paradigm/.md), создающим это действие.

Действие может возвращать *результат* — объект фиксированного класса или набор таких объектов по дополнительным параметрам — вызывающему коду через [оператор выхода](/ru/Exit_RETURN/.md) внутри тела действия.

### Язык[​](#язык "Прямая ссылка на этот заголовок")

Для добавления нового действия в систему используется [инструкция `ACTION`](/ru/ACTION_statement/.md).

### Примеры[​](#примеры "Прямая ссылка на этот заголовок")

```
CLASS Item;
inStock (Item i) = DATA BOOLEAN (Item);

// действие описывает эффект — здесь изменение хранимого свойства
markInStock (Item i)  {
    inStock(i) <- TRUE;
}

// действие может объявлять результат, возвращаемый вызывающему коду
getDescription (Item i) ABSTRACT STRING[100];
```
