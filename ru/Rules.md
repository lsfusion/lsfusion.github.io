...
Ветки `language`, `paradigm` и `how-to` содержат справочные материалы и ищутся инструментом `lsfusion_retrieve_docs`. То, что ассистент не извлёк отдельную статью из этих веток, само по себе не является нарушением настоящих правил.
...
Эта статья НЕ содержит перечисленных ниже правил. Каждая строка — это отдельная статья, читаемая целиком вызовом `lsfusion_get_guidance(rules='<имя>')` с именем из первого столбца. Столбец **Читать перед тем, как** — это обязанность, а не пожелание.
...
1. Пояснение в таблице выше — это строка указателя, а не правило. Ассистент НЕ ДОЛЖЕН делать вывод о содержании статьи по её пояснению и НЕ ДОЛЖЕН действовать по пояснению вместо статьи.

2. Перед тем как делать то, что описано в триггере строки, ассистент ОБЯЗАН вызвать `lsfusion_get_guidance(rules='<имя>')` для этой строки, прочитать полученное и применить каждое правило с учётом указанной для него силы. Одного раза на область за сессию достаточно.

3. Триггер, сработавший посреди работы, подлежит исполнению в этот момент. Ассистент ОБЯЗАН прочитать статью до написания конструкции и НЕ ДОЛЖЕН откладывать это на последующую проверку.

4. Перед завершением задачи ассистент ОБЯЗАН сверить фактически написанные конструкции с таблицей и ОБЯЗАН прочитать каждую статью, чей триггер сработал, но которая не была прочитана.

5. Область, которую ассистент не прочитал, — это не область без правил. Ассистент НЕ ДОЛЖЕН утверждать, подразумевать или исходить из того, что к области, статью которой он не читал, не применяется ни одно правило, и НЕ ДОЛЖЕН сообщать о пробеле в документации той статьи, которую он не читал.

6. Каждая статья приходит целиком, между строками `=== BEGIN ... ===` и `=== END ... ===`. Если строки END нет, копия обрезана: ассистент ОБЯЗАН восстановить полный текст — прочитав сохранённый файл целиком, если клиент его сохранил, — прежде чем что-либо из него применять.

7. Если вызов завершился ошибкой, ассистент ОБЯЗАН повторить его один раз. Если ошибка повторилась, ассистент ОБЯЗАН сообщить пользователю, правила какой области прочитать не удалось, и НЕ ДОЛЖЕН выдавать полученный код за проверенный на соответствие правилам.
...
2. Ассистент ОБЯЗАН извлечь определения и синтаксис для этих типов элементов до редактирования. Этот поиск охватывает только `language`, `paradigm` и `how-to`. Правила области не ищутся: они читаются целиком, по имени, согласно карте и разделу об обязательном чтении выше. Запрос СЛЕДУЕТ формулировать кратко и в технических терминах, по возможности по-английски, называя ключевое слово lsFusion, если оно известно. Если несколько независимых потребностей известны заранее, ассистенту СЛЕДУЕТ передать их одним вызовом — `query` принимает список различных запросов, — а не делать вызов на каждую. Ассистент ОБЯЗАН искать последовательно, когда результат одного поиска определяет или уточняет следующий запрос, и НЕ ДОЛЖЕН объединять в пакет разные формулировки одной потребности. `exclude_ids` продолжает информационную потребность теми `id`, что на руках, и действует на все запросы пакета; переформулировка, другой вопрос или пакет других запросов идут без него — иначе фильтр отбросит как раз тот чанк, который бы ответил.
...
4. Выражение, чья запятая НЕ заключена в собственные скобки — `OVERRIDE a, b`, `CONCAT sep, a, b`, `GROUP CONCAT expr, sep` — НЕ ДОЛЖНО попадать прямо в список с разделением запятыми (`PROPERTIES`, `EXPORT FROM`, `JSON FROM`, `ORDER`, списки групп объектов и параметров): эта запятая читается как разделитель списка, и список молча перекраивается. Сгруппируйте его или назовите свойством — той формой, какую принимает внешний блок. Запятая внутри собственных скобок обычного вызова безопасна.
