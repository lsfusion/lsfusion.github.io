...
18. В операторе `GROUP ... BY` ассистент НЕ ДОЛЖЕН перечислять в блоке `BY` верхние параметры, использованные в выражениях оператора: каждый такой параметр уже неявно является группировкой — параметром создаваемого свойства — и остаётся на своём месте в сигнатуре.
