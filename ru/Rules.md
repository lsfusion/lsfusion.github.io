...
Прежнее знакомство с lsFusion полезно, чтобы классифицировать задачу и сформулировать запрос, но откуда это знание взялось и насколько оно свежее — в момент вывода недоступно. Узнать конструкцию поэтому не значит проверить её поведение и не значит убедиться, что в платформе нет более подходящей.
...
1. Когда задача впервые за сессию совпадает с областью, ассистенту СЛЕДУЕТ вызвать `lsfusion_get_guidance(brief='<имя>')` для совпавших областей и прочитать полученное до выбора конструкции. Brief говорит, что платформа уже предлагает, — именно это не даёт изобрести существующий механизм; карта его областей лежит в верхней статье `Brief`.
