...
4. Правила 1-3 описывают форму события-действия `WHEN <условие> DO <целевое> <- <выражение>`. В форме реактивного присваивания `<целевое> <- <выражение> WHEN <условие>` защита НЕ ДОЛЖНА проверять `CHANGED(<целевое>)`: целевое свойство стало бы зависеть от собственного изменения, образуя цикл `<целевое>` -> `CHANGED(<целевое>)` -> `<целевое>`.

   Чтобы подставить значение по умолчанию, но всё же уступать явному изменению, записывайте это в виде самозащищённой формы события-действия (`WHEN [LOCAL] <условие> AND NOT CHANGED(<целевое>) DO <целевое> <- <выражение>`), а не реактивного присваивания.
