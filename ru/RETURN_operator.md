...
Оператор `RETURN` - создание [действия](/ru/Actions/.md), реализующего [выход](/ru/Exit_RETURN/.md) из действия, созданного [оператором `EXEC`](/ru/Call_EXEC/.md).
...
Оператор `RETURN` создает действие, которое выходит из наиболее вложенного [вызова действия](/ru/Call_EXEC/.md). Если задано `resultExpr`, значение этого выражения становится результатом этого вызова. Иначе вызов просто завершается, не возвращая значения.
