...
Оператор `EXTERNAL` - создание [действия](/ru/Actions/.md), выполняющего [обращение к внешней системе](/ru/Access_to_an_external_system_EXTERNAL/.md).
...
* `HTTP`, `TCP`, `UDP`, `SQL`, `LSF`, `DBF`
  ...
  Ключевые слова. Задают тип внешнего вызова; семантика каждого описана в статье [Обращение к внешней системе](/ru/Access_to_an_external_system_EXTERNAL/.md).
...
* `charsetLiteral`
  ...
  [Строковый литерал](/ru/Literals/.md#strliteral) с кодировкой `.dbf`-файла. По умолчанию `UTF-8`.

* `connectionStrExpr`

  [Выражение](/ru/Expression/.md). `HTTP`: строка http-запроса. `TCP` / `UDP`: `host:port` целевого сокета. `SQL`: строка подключения к СУБД. `LSF`: URL сервера приложений или веб-сервера lsFusion. `DBF`: путь к `.dbf`-файлу.

* `bodyStrExpr`

  [Выражение](/ru/Expression/.md). Строка BODY с подстановкой параметров через `$N`. Для HTTP-методов с телом все параметры, оставшиеся после подстановки в URL, должны быть использованы внутри этой строки, иначе вызов падает. Для `GET` `BODYURL` не действует, а оставшиеся параметры молча отбрасываются.

* `bodyParamNameExpr1, ..., bodyParamNameExprK`

  Список [выражений](/ru/Expression/.md). Каждое вычисляется в имя BODY-части в форме `'name'` или `'name;filename'` (часть после `;` задаёт имя файла multipart-части). Без `BODYPARAMNAMES` части BODY получают автоматические имена `param0`, `param1`, ... (файловые части без явного filename аналогично получают `file0`, `file1`, ...).

* `bodyParamHeadersPropertyId1, ..., bodyParamHeadersPropertyIdK`

  Список [идентификаторов свойств](/ru/IDs/.md#propertyid). Каждое свойство имеет один параметр строкового класса (имя заголовка) и значение строкового класса (значение заголовка). Без `BODYPARAMHEADERS` дополнительные заголовки к частям BODY не добавляются.

* `headersPropertyId`, `headersToPropertyId`

  [Идентификаторы свойств](/ru/IDs/.md#propertyid). Каждое свойство имеет один параметр строкового класса (имя заголовка) и значение строкового класса (значение заголовка). Без `HEADERS` собственные заголовки запроса не устанавливаются; без `HEADERSTO` заголовки ответа не сохраняются.

* `cookiesPropertyId`, `cookiesToPropertyId`

  [Идентификаторы свойств](/ru/IDs/.md#propertyid). Каждое свойство имеет один параметр строкового класса (имя cookie) и значение строкового класса (значение cookie). Без `COOKIES` собственные cookies не отправляются; без `COOKIESTO` cookies не сохраняются.
...
* `lsfExecType`
  ...
  Ключевое слово. Определяет [способ задания](/ru/Access_from_an_external_system/.md#actiontype) действия:
...
* `execStrExpr`
  ...
  [Выражение](/ru/Expression/.md). `SQL`: SQL-команда(ы) — выражение, заканчивающееся на `.sql`, трактуется как путь к ресурсу classpath, содержимое которого используется как команда. `LSF`: имя действия или код, в зависимости от `lsfExecType`.
...
* `propertyId1, ..., propertyIdM`
  ...
  Список [идентификаторов свойств](/ru/IDs/.md#propertyid) (без параметров), в которые будут записаны полученные результаты.
