...
* Обработка событий изменений (`CHANGEWYS` и `CHANGE`) [по умолчанию](/ru/Form_events/.md#default) для композиции
  ...
  * CHANGE
    ...
    У пользователя [запрашивается](/ru/Form_events/.md#queryValue) объект класса значения свойства, которое используется в качестве аргумента, после чего значение этого свойства изменяется на полученный объект
...
Также как и в остальных операторах ввода значения, в этом операторе можно задавать [основное и альтернативное](/ru/Value_input/.md#result) действия. Первое вызовется, если ввод был успешно завершен, второе - в обратном случае (если ввод был отменен). Соответственно, именно эти два действия в этом операторе отвечают за обработку значений запроса.
