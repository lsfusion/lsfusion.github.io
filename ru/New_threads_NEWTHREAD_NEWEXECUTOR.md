...
Сам по себе оператор создания нового потока не создаёт новую [сессию изменений](/ru/Change_sessions/.md) и не открывает отдельное SQL-соединение. В каком окружении исполняется тело потока, зависит от службы выполнения (см. ниже).

### Где исполняется поток[​](#где-исполняется-поток "Прямая ссылка на этот заголовок")
...
#### Серверное исполнение[​](#серверное-исполнение "Прямая ссылка на этот заголовок")
