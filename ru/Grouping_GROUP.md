...
* `EQUAL` является частным случаем агрегирующей функции `MAX` (или `MIN`), добавляя [ограничение](/ru/Constraints/.md) на одинаковость значения операнда агрегирующей функции внутри каждой группы. Классом результата является класс выражения-операнда.
...
Для объявления свойства, реализующего группировку, используется [оператор `GROUP`](/ru/GROUP_operator/.md).
