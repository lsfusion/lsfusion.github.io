...
Оператор `FORMULA` - создание [свойства](/ru/Properties/.md), реализующего [пользовательскую формулу](/ru/Custom_formula_FORMULA/.md).
...
`FORMULA` — [контекстно-независимый](/ru/Property_operators/.md#contextindependent) оператор-свойство: он не может стоять внутри [выражений](/ru/Expression/.md). Использовать его можно в правой части `=`-объявления или анонимно внутри скобок в применении [оператора `JOIN`](/ru/JOIN_operator/.md).

Что именно создаёт оператор — результирующий класс по умолчанию, ослабления обработки `NULL` и табличный режим — описано в абстракции [пользовательской формулы](/ru/Custom_formula_FORMULA/.md); эта статья описывает только то, как эти варианты записываются.
