...
Отдельных операторов остатка от деления, целочисленного деления и возведения в степень нет — эти операции выполняют свойства `mod[…, …]`, `divideInteger[…, …]` и `power[…, …]` системного модуля [`Utils`](/ru/System_Utils.md).
