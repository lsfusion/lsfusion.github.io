...
* `orderExpr1, ..., orderExprL`
  ...
  Список [выражений](/ru/Expression.md), по которым производится сортировка экспортируемых данных. Выражения произвольны: они не обязаны входить в список `propertyExpr1, ..., propertyExprN`. Выражение сортировки добавляется во внутренний запрос скрытой колонкой и в результат экспорта не попадает.
...
* `rootExpr`
  ...
  [Выражение](/ru/Expression.md), значение которого используется как имя корневого XML-элемента. Значение выражения должно быть строкового класса. Если не указано, используется имя экспортируемой формы (`export` при экспорте свойств напрямую). Применяется только для экспорта в **XML**.
