...
### Расширенная форма[​](#расширенная-форма "Прямая ссылка на этот заголовок")
...
Так как простая форма оператора семантически очень похожа на оператор [разбиения / упорядочивания](/ru/Partitioning_sorting_PARTITION_..._ORDER/.md), для объявления свойства, реализующего простое распределение также используется [оператор `PARTITION`](/ru/PARTITION_operator/.md).

Для расширенной формы используется [оператор `UNGROUP`](/ru/UNGROUP_operator/.md).
