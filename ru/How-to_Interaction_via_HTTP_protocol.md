...
Ответ разбирается при помощи [оператора `IMPORT`](/ru/Data_import_IMPORT/.md), который раскладывает соответствующие параметры в свойства `code` и `message` соответственно. В случае ошибки пользователю выдается сообщение с текстом ошибки.
...
Так как свойство имеет название `createCity` и расположено в [модуле](/ru/Modules/.md) с пространством имен `Location`, то url, на котором будет принят запрос, имеет следующий вид:
...
Если не находится страна с соответствующим кодом, то формируется JSON файл с содержанием, описанном в предыдущем примере, и вызывается [оператор `RETURN`](/ru/Exit_RETURN/.md), чтобы прервать выполнение. В качестве ответа, по умолчанию, используется значение, хранящееся в свойстве `exportFile`.
...
Для `order` не создается свой тэг, так как значение объекта передается параметром для оператора `EXPORT`.<br /><!-- -->В данном примере, ответ полученный на HTTP-запрос игнорируется.
...
Аналогично [**Примеру 4**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-4).
...
В отличие от предыдущего примера создаем [группу](/ru/Groups_of_properties_and_actions/.md) `order`, при помощи [оператора `GROUP`](/ru/GROUP_operator/.md). При объявлении формы в эту группу записываем все свойства для заказа и объект `detail`. Результирующий JSON будет выглядеть следующим образом:
...
Аналогично [**Примеру 5**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-5).
...
### Условие[​](#условие-6 "Прямая ссылка на этот заголовок")
...
### Условие[​](#условие-7 "Прямая ссылка на этот заголовок")
