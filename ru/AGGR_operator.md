...
Оператор `AGGR` может появляться только как определение свойства верхнего уровня в [инструкции `=`](/ru/=_statement/.md); его нельзя использовать внутри [выражения](/ru/Expression/.md), внутри других операторов или внутри формы `[= ]` [оператора `JOIN`](/ru/JOIN_operator/.md).
