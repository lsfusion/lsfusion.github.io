...
Реализация всегда записывается как тело действия после знака `+`. Инструкция [`+=`](/ru/plus_equals_statement.md) к действиям не относится: она добавляет реализацию абстрактного свойства и принимает выражение, поэтому запись `abstractAction(...) += { ... }` не разбирается.
