...
Оператор *рекурсии* - оператор, создающий [свойство](/ru/Properties/.md), которое последовательно выполняет две операции:
...
1. Рекурсивно строит промежуточное свойство (result) с дополнительным первым параметром (номером операции) следующим образом:
   ...
   2. `result(i+1, o1, o2, ..., oN) = step(o1, ..., oN, $o1, $o2, ..., $oN) IF result(i, $o1, $o2, ..., $oN)`, где `step` - свойство *шага*, а `$o1, ..., $oN` обозначают значения параметров на предыдущей итерации (в исходном коде они записываются как `$name` — см. [оператор `RECURSION`](/ru/RECURSION_operator/.md)).

2. Для всех значений полученного свойства вычисляет заданную [агрегирующую функцию](/ru/Set_operations/.md#func) в разрезе всех его параметров, за исключением номера операции.
...
Для объявления свойства, реализующего рекурсию, используется [оператор `RECURSION`](/ru/RECURSION_operator/.md).
