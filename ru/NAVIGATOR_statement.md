...
Инструкция `NAVIGATOR` - управление и настройка [навигатора](/ru/Navigator/.md).
...
* `name`
  ...
  Имя элемента навигатора. В инструкции создания является [простым идентификатором](/ru/IDs/.md#id) и должно быть уникальным в пределах текущего [пространства имен](/ru/Naming/.md#namespace), в остальных инструкциях является [составным идентификатором](/ru/IDs/.md#cid). Имя может быть не задано при создании элемента-формы или элемента-действия, в этом случае в качестве имени элемента будет использовано имя формы либо имя действия соответственно. Элемент в любом случае создаётся в текущем пространстве имён, поэтому его [каноническое имя](/ru/Naming/.md#canonicalname) складывается из этого пространства имён и имени элемента, а не из пространства имён формы или действия, из которых он создан.

* `caption`

  Заголовок элемента навигатора. [Строковый литерал](/ru/Literals/.md#strliteral). В инструкции создания, если заголовок не задан, то заголовком будет являться имя создаваемого элемента. В остальных инструкциях изменяет существующий заголовок.

* `actionName`

  [Идентификатор действия](/ru/IDs/.md#propertyid), для которого будет создан элемент навигатора. Действие не должно принимать на вход параметры.
...
* `SCHEDULE PERIOD intPeriod [FIXED] actionOperator`
  ...
  * `actionOperator`
    ...
    [Оператор-действие](/ru/Action_operators/.md), задающий выполняемое действие. Параметры в нём недоступны.

### Опции[​](#опции "Прямая ссылка на этот заголовок")

* `WINDOW windowName [PARENT]`

  Указание [окна](/ru/Navigator_design/.md), в котором будут отображаться потомки этого элемента навигатора (если для них не указано другое окно) и сам элемент, если указано ключевое слово `PARENT`.
...
* `imageSetting`
  ...
  * `IMAGE [imageExpr]`
    ...
    [Указание иконки](/ru/Icons/.md#manual) элемента навигатора. Если `imageExpr` не указывается, то происходит переключение в режим [автоматической установки](/ru/Icons/.md#auto) иконки.

    * `imageExpr`

      [Выражение](/ru/Expression/.md), строковое значение которого определяет иконку.
...
* `CHANGEKEY key [showSetting]`
  ...
  Указание [комбинации клавиш](/ru/Form_events/.md#keyboard), при нажатии которой будет выбран этот элемент навигатора.

  * `key`

    [Строковый литерал](/ru/Literals/.md#strliteral), описывающий комбинацию клавиш. Параметры режимов (`editing=`, `showing=` и т.п.), допустимые после `;` в комбинациях [свойств и действий](/ru/Property_options/.md), для элементов навигатора в веб-клиенте не применяются. Комбинация обрабатывается глобально, но после обработчиков открытой формы: одноимённая комбинация свойства или действия формы имеет приоритет.
