...
Оператор `UNGROUP` - создание [свойства](/ru/Properties/.md), реализующее [распределение](/ru/Distribution_UNGROUP/.md) в расширенной форме.
  ...
  * `PROPORTION`
    ...
    * `ROUND(digits)`
      ...
      * `digits`
        ...
        [Целочисленный литерал](/ru/Literals/.md#intliteral), указывающий количество знаков после запятой.
...
* `exPropertyId`
  ...
  [Идентификатор](/ru/IDs/.md#propertyid) свойства, которое для распределения с типом `PROPORTION` определяет пропорцию, а с типом `LIMIT` определяет лимиты. Значение этого свойства должно быть числового типа, а количество параметров должно совпадать с количеством группировок во втором блоке `BY`.
