...
Аналогично [**Примеру 1**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-1). Для заказа задан статус.
...
[Условие простого события проверяется и на объектах, удаляемых в сессии](/ru/Simple_event/.md): при удалении заказа его первичные свойства сбрасываются в `NULL`, поэтому без `o IS Order` событие сработало бы и при удалении — для уже удалённого заказа создалась бы запись истории. Её связь `order(l)` при применении тоже сбросилась бы в `NULL` (значение недопустимо для удалённого объекта), и `NONULL` заблокировал бы удаление заказа, хотя запрет удаления нигде не объявлен. Для удалённого заказа `o IS Order` даёт `NULL`, и событие не наступает. Если удаление, наоборот, нужно журналировать, используется событие с модификатором `DROPPED` и чтением прежних значений через `PREV`, как в [**Примере 4**](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-4).
