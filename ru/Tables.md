...
По умолчанию, для каждой таблицы строится уникальный [индекс](/ru/Indexes/.md) по ее ключевым полям `key0`, `key1`, ..., `keyN` с именем `pk_<идентификатор таблицы>`, где `N` - количество ключевых полей в таблице минус `1`. Также автоматически добавляются индексы по ключевым полям `keyK`, ..., `keyN` с именами `<идентификатор таблицы>_keyK_..._keyN_idx`, для всех `K` от `1` до `N`.
