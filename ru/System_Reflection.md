...
`Reflection` — [системный модуль](/ru/System_modules.md), отражающий собственные метаданные приложения — навигатор, формы, свойства, действия и таблицы физической модели — в виде классов, свойств и форм, так что работающая логика может исследовать собственную структуру. Подключается через `REQUIRE Reflection` (`System` и `Authentication` тянутся автоматически).

Модуль хранит по одному объекту на каждый элемент метаданных (по одному `Property` на свойство, по одному `Form` на форму, по одному `Table` на таблицу и т. д.), наполняемому платформой при запуске (при [облегчённом запуске](/ru/Launch_parameters.md) бо́льшая часть этой синхронизации — всё, кроме таблиц, — пропускается, и формы, элементы навигатора, свойства и действия, добавленные после последнего полного запуска, в модуле отсутствуют). Свойства-поиски восстанавливают объект по каноническому имени или SID; статистика, признаки хранения и пользовательские настройки колонок привязаны к этим объектам; а формы в конце модуля показывают их под папкой навигатора `metadata`.

### Группы свойств[​](#группы-свойств "Прямая ссылка на этот заголовок")

[Свойства и действия](/ru/User_classes.md) организованы в дерево групп.
...
### Метаданные навигатора[​](#метаданные-навигатора "Прямая ссылка на этот заголовок")

Дерево [навигатора](/ru/Navigator.md) представлено классом `NavigatorElement` и двумя его наследниками.
...
### Метаданные форм[​](#метаданные-форм "Прямая ссылка на этот заголовок")

| Класс / свойство                        | Что хранит                                                     |
| --------------------------------------- | -------------------------------------------------------------- |
| `Form`                                  | по одному объекту на [форму](/ru/Forms.md)                     |
...
### Метаданные действий и свойств[​](#метаданные-действий-и-свойств "Прямая ссылка на этот заголовок")
...
### Отрисовки свойств и настройки колонок[​](#отрисовки-свойств-и-настройки-колонок "Прямая ссылка на этот заголовок")
...
### Физическая модель[​](#физическая-модель "Прямая ссылка на этот заголовок")

Классы физической модели отражают [таблицы](/ru/Tables.md) базы данных, их ключи и колонки.
...
* [Оператор `RECURSION`](/ru/RECURSION_operator.md) — вычисляет `level[PropertyGroup, PropertyGroup]` и `level[NavigatorElement, NavigatorElement]`: не `NULL` для предка и самого объекта, значение — `2` в степени расстояния.
* [Оператор `INTERNAL`](/ru/INTERNAL_operator.md) — стоит за служебными действиями физической модели, реализованными на Java.

### Связано[​](#связано "Прямая ссылка на этот заголовок")

* [`System modules`](/ru/System_modules.md) — общий перечень модулей платформы.
* [`Навигатор`](/ru/Navigator.md) — дерево навигатора, отражаемое классом `NavigatorElement`.
* [`Формы`](/ru/Forms.md) — что такое форма и как `Form` / `PropertyDraw` / `GroupObject` соответствуют ей.
* [`Таблицы`](/ru/Tables.md) — физические таблицы, отражаемые классами `Table` / `TableKey` / `TableColumn`.
* [`Индексы`](/ru/Indexes.md) — индексы таблиц.
* [`Материализации`](/ru/Materializations.md) — материализованные колонки, пересчитываемые служебными действиями.
* [`System`](/ru/System_System.md) — корневой модуль, подключающийся автоматически.
* [`Authentication`](/ru/System_Authentication.md) — пользователи и контакты, подключаемые через `REQUIRE`; пользовательские настройки форм хранятся по `CustomUser`.
* [`Security`](/ru/System_Security.md) — задаёт права доступа на отражаемые здесь элементы навигатора, свойства и действия.
* [`Service`](/ru/System_Service.md) — служебные действия над отражаемыми здесь таблицами и материализованными колонками.
