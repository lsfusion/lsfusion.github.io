...
Создавать объекты можно также внутри [действия цикла](/ru/Loop_FOR/.md): тогда на каждой итерации создается один объект, видимый в теле цикла, — см. соответствующую [опцию](/ru/Loop_FOR/.md#addobject) в статье про цикл.

### Язык[​](#язык "Прямая ссылка на этот заголовок")

Для объявления действия, создающего объекты, используется [оператор `NEW`](/ru/NEW_operator/.md). Для создания, привязанного к циклу, используется опция `NEW` [оператора `FOR`](/ru/FOR_operator/.md).
