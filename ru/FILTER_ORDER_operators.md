# Операторы фильтров и порядков формы

Операторы `ORDER`, `FILTER`, `FILTERGROUP`, `FILTER PROPERTY` (и их варианты `ORDERS`, `FILTERS`, `FILTERGROUPS`, `FILTERS PROPERTY`) - создание [действий](/ru/Actions/.md), которые применяют или читают текущие пользовательские фильтры и порядки элементов открытой формы.
...
Операторы работают с [интерактивным представлением](/ru/Interactive_view/.md) открытой формы и делятся на две группы:
...
Если блок `FROM` или `TO` не указан, по умолчанию используется соответствующее свойство системного модуля [`UserEvents`](/ru/System_UserEvents/.md) (`orders`, `filters`, `filterGroups`, `filtersProperty`), через который эти операторы обычно и вызываются.

### Параметры[​](#параметры "Прямая ссылка на этот заголовок")

* `groupObjectId`

  [Идентификатор группы объектов](/ru/IDs/.md#groupobjectid) на форме.

* `filterGroupId`

  Имя [группы фильтров](/ru/Interactive_view/.md#filtergroup) на форме, квалифицированное формой.

* `formPropertyId`

  [Идентификатор свойства или действия на форме](/ru/IDs/.md#formpropertyid).

* `expr`

  [Выражение](/ru/Expression/.md), значение которого применяется к элементу формы. Его класс определяет вид сериализации (см. «Описание»).

* `propId`

  [Идентификатор свойства](/ru/IDs/.md#propertyid) без параметров, в которое записывается прочитанное значение.
