...
*Операторы изменений* позволяют определить произошли ли для заданного свойства в текущей сессии те или иные виды изменений. Каждый из них создает свойство [класса `BOOLEAN`](/ru/Built-in_classes/.md), которое возвращает `TRUE`, когда соответствующее изменение произошло, и `NULL` в противном случае. Все эти операторы являются производными от [оператора предыдущего значения (`PREV`)](/ru/Previous_value_PREV/.md), однако для улучшения читабельности и производительности рекомендуется использовать именно их.
...
В [событийном режиме](/ru/Events/.md#change) эти операторы возвращают изменения с момента предыдущего возникновения события (а точнее, окончания выполнения всех его обработок) вместо изменений с начала сессии.

### Язык[​](#язык "Прямая ссылка на этот заголовок")

Для объявления свойства при помощи операторов изменений, используются следующие [синтаксические конструкции](/ru/Change_operators/.md).
