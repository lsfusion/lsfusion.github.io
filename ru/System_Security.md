...
`Security` — [системный модуль](/ru/System_modules.md), задающий модель управления доступом: роли пользователей, назначение ролей, права на свойства, действия, группы свойств и элементы навигатора, настройки форм и клиента по ролям, ограничения памяти клиента и форму поиска по навигатору. Он лежит в основе управленческой статьи [`Политика безопасности`](/ru/Security_policy.md), интерфейс ролей и доступа которой построен на объявленных здесь классах, свойствах и формах. Подключается через `REQUIRE Security` (`System`, `SystemEvents`, `Email` и `Utils` тянутся автоматически как его зависимости).

### Роли[​](#роли "Прямая ссылка на этот заголовок")
...
### Назначение ролей[​](#назначение-ролей "Прямая ссылка на этот заголовок")
...
### Системные роли[​](#системные-роли "Прямая ссылка на этот заголовок")
...
### Права[​](#права "Прямая ссылка на этот заголовок")
...
### Настройки форм и клиента[​](#formSettings "Прямая ссылка на этот заголовок")

Эти признаки задаются по роли и объединяются по ролям пользователя.

| Свойство                                  | Что делает                                                                                                      |
| ----------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `forbidDuplicateForms[UserRole]`          | при открытии формы из [навигатора](/ru/Navigator.md) активировать уже открытую закладку этой формы вместо новой |
...
#### Формы по умолчанию[​](#формы-по-умолчанию "Прямая ссылка на этот заголовок")
...
### Запуск клиента и память[​](#запуск-клиента-и-память "Прямая ссылка на этот заголовок")
...
### Поиск по навигатору[​](#поиск-по-навигатору "Прямая ссылка на этот заголовок")
...
### Расширения рефлексии[​](#расширения-рефлексии "Прямая ссылка на этот заголовок")
...
* [Оператор `META`](/ru/META_statement.md) — механизм метакода, стоящий за порождаемыми семействами прав на свойства (`@initPropertyPermission`) и точкой расширения настроек по роли (`@addRoleApplicationSetting`).

### Связано[​](#связано "Прямая ссылка на этот заголовок")

* [`Политика безопасности`](/ru/Security_policy.md) — управленческая статья о настройке ролей и доступа; данный модуль служит её основой из стандартной библиотеки.
* [`System modules`](/ru/System_modules.md) — общий список модулей платформы.
* [`Authentication`](/ru/System_Authentication.md) — пользователи, контакты и вход в систему, чьим классам `User` / `CustomUser` назначаются роли.
* [`Reflection`](/ru/System_Reflection.md) — метаданные о навигаторе, формах и свойствах, на которые задаются права.
