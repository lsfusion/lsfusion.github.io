...
### Обслуживание базы данных[​](#обслуживание-базы-данных "Прямая ссылка на этот заголовок")
...
#### Многопоточные варианты[​](#многопоточные-варианты "Прямая ссылка на этот заголовок")
...
### Режимы базы данных[​](#режимы-базы-данных "Прямая ссылка на этот заголовок")
...
### Режим запуска сервера[​](#режим-запуска-сервера "Прямая ссылка на этот заголовок")

Эти признаки без параметров отражают [параметры запуска](/ru/Launch_parameters.md) сервера и позволяют логике учитывать режим, в котором он запущен.

| Свойство         | Что означает                                                                                                                                                |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `inDevMode[]`    | сервер запущен в режиме разработки (`-Dlsfusion.server.devmode`)                                                                                            |
| `isLightStart[]` | сервер запущен в режиме облегчённого запуска (`-Dlsfusion.server.lightstart`): синхронизация [рефлексии](/ru/System_Reflection.md), кроме таблиц, пропущена |
| `inTestMode[]`   | сервер запущен в тестовом режиме (`-Dlsfusion.server.testmode`)                                                                                             |

### Виртуальная машина и память[​](#виртуальная-машина-и-память "Прямая ссылка на этот заголовок")
...
### Перезапуск сервера[​](#перезапуск-сервера "Прямая ссылка на этот заголовок")
...
### Настройки[​](#настройки "Прямая ссылка на этот заголовок")

Механизм `Setting` хранит именованные общесистемные настройки, значение которых можно переопределить по [роли пользователя](/ru/System_Authentication.md). Объект `Setting` опознаётся по `name[Setting]` (`ISTRING`); `setting[ISTRING]` ищет настройку по имени.
...
### Пользовательская диагностика[​](#пользовательская-диагностика "Прямая ссылка на этот заголовок")
...
| `remoteLoggerDebugEnabled[User]`      | журналирование удалённых вызовов на уровне debug: в `server-remote.log` удалённые вызовы пользователя пишутся с аргументами независимо от длительности, а не только вызовы дольше [параметра](/ru/Working_parameters.md) `remoteLogTime` |
...
### Пользовательские настройки выполнения[​](#пользовательские-настройки-выполнения "Прямая ссылка на этот заголовок")

Эти свойства на пользователе настраивают выполнение запросов и поведение клиента. `execEnv[User]` (класс `TypeExecEnv` с объектами `materialize`, `disablenestloop`, `none`) выбирает среду выполнения запросов и проталкивает выбранный `id[TypeExecEnv]` в сервер тем же набором `set…` / `refresh…` / `WHEN CHANGED`, что и у признаков диагностики; `nameExecEnv[User]` — его подпись. Пользователю, которому среда не выбрана, достается та, что задана [рабочим параметром](/ru/Working_parameters.md) `defaultTypeExecuteEnvironment`.
...
### Клиентские логи и дампы[​](#клиентские-логи-и-дампы "Прямая ссылка на этот заголовок")
...
### Клиентский интерфейс по пользователям[​](#клиентский-интерфейс-по-пользователям "Прямая ссылка на этот заголовок")
...
### Масштабирование базы данных[​](#database-scaling "Прямая ссылка на этот заголовок")
...
### API серверных настроек[​](#api-серверных-настроек "Прямая ссылка на этот заголовок")
...
* [Оператор `INTERNAL`](/ru/INTERNAL_operator.md) — объявляет действия обслуживания, режимов, диагностики, настроек и масштабирования над их Java-классами реализации.

### Связано[​](#связано "Прямая ссылка на этот заголовок")

* [`System modules`](/ru/System_modules.md) — общий перечень модулей платформы.
* [`Process monitor`](/ru/Process_monitor.md) — представление работающих процессов, которое поддерживают диагностика и дампы этого модуля.
* [`Journals and logs`](/ru/Journals_and_logs.md) — логи сервера, которые наполняют пользовательские признаки журналирования.
* [`Working parameters`](/ru/Working_parameters.md) — рабочие параметры и настройки, конфигурируемые этой административной поверхностью.
* [`Backup and restore`](/ru/Backup_restore.md) — резервное копирование базы данных рядом с действиями обслуживания базы здесь.
* [`Reflection`](/ru/System_Reflection.md) — модуль метаданных, на котором держатся административные формы.
* [`SystemEvents`](/ru/System_SystemEvents.md) — модуль событий жизненного цикла сервера, подключаемый через `REQUIRE`.
* [`Authentication`](/ru/System_Authentication.md) — пользователи и роли, относительно которых хранятся настройки и предпочтения интерфейса.
