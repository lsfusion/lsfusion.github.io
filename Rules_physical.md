...
A table per set of key classes, a property placed into one by `TABLE`, and the read result materialized — the line-level `sum` is left computed, since rule 3 says the chain's intermediate step is not what gets stored. The composite index puts the equality field before the range one, as rule 3 of the index rules asks.
...
3. Without the `STORED PROPERTY` entry, on the next server start the old column is renamed to `_DELETED_` plus its old database name — or dropped outright if a column by that name is already there — and a fresh empty column is created for the new name, so all existing values of the property are lost. The assistant MUST NOT rename or move a `DATA` property to another namespace without adding this entry.
