...
The groups to iterate over are defined by the `BY` block, so the [`GROUP` operator](/GROUP_operator.md) is written as an inline definition in the brackets of the [`JOIN` operator](/JOIN_operator.md), and its argument declares the new parameter `b`: the loop iterates over the books that have lines in the order. The order `o` is used inside the brackets, so it is passed automatically and is not listed among the arguments. The parameter `d` declared inside the brackets belongs to the `GROUP` operator and is not available in the loop.
