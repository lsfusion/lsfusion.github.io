...
## Data export map[​](#data-export-map "Direct link to Data export map")

A detailed map of a single area — exporting data from the system to a file. The overall element map is in [Brief](/Brief/.md), the full syntax is in the [`EXPORT` operator](/EXPORT_operator/.md), and the mechanism is described in [Data export](/Data_export_EXPORT/.md).

This article is not part of the set always passed to the assistant; it is requested when needed.

## Two export sources[​](#two-export-sources "Direct link to Two export sources")

### Exporting properties[​](#exporting-properties "Direct link to Exporting properties")
...
### Exporting a form[​](#exporting-a-form "Direct link to Exporting a form")

```
EXPORT formName [OBJECTS objName = expr, ...] [format] [TOP ...] [OFFSET ...] [TO ...]
```

The form is opened in the [structured view](/Structured_view/.md), so its object group hierarchy is carried into the result for **JSON** and **XML**. For the flat formats (**CSV**, **XLS**, **XLSX**, **DBF**, **TABLE**) each object group is exported to a separate file. Objects whose values are fixed in the `OBJECTS` block act as filters and do not participate in building the group hierarchy.

## Formats and extensions[​](#formats-and-extensions "Direct link to Formats and extensions")
...
## Format options and their defaults[​](#format-options-and-their-defaults "Direct link to Format options and their defaults")
...
* When a form is exported to a flat format, the destinations are set per object group: `TO groupId1 = propertyId1, ...` — groups not listed are not exported, and exporting a form to a single file in a flat format is not supported; the [empty object group](/Static_view/.md#empty) is named `root`.
